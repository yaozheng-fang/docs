...
<Steps>
  ...
  <Step title="部署">
    ...
    `agentkit deploy` 全程无需 flag，读 `agentkit.yaml` 依次执行：构建并部署运行时（镜像运行 veADK 前端，同源提供 UI 与智能体 API，`custom_jwt` 由用户池自动派生），再在 VeFaaS 上部署公网前端 BFF（复用已有 serverless 网关，自动获取客户端 secret、自动登记回调 `<前端地址>/oauth2/callback`）。
  </Step>
  <Step title="打开使用">
