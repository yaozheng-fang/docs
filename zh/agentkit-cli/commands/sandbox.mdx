...
<Note>
各子命令共有一组会话与工具相关标志：`-s, --session <id>`（会话 ID，存在则复用，否则新建）、`--tool-id <id>`（沙箱工具 ID）、`--type <type>`（沙箱类型 `code` \| `skill`，默认 `code`）、`--auto-create`（无可用沙箱工具时自动创建）、`-r, --region <region>`（Volcengine 区域）、`-p, --project <name>`（AgentKit 项目，默认 `default`）。下方表格逐命令完整列出。

未传 `--tool-id` 时，CLI 会在项目内查找该类型的沙箱工具：**恰好一个时自动选用；存在多个时命令会失败并要求显式传入 `--tool-id`**；一个都没有时需配合 `--auto-create` 自动创建。项目内已有多个 `code` 工具属常见情况，此时务必用 `agentkit tool list` 查到目标 ID 后通过 `--tool-id` 指定。
</Note>
...
| 标志/参数 | 说明 | 默认值 |
| --- | --- | --- |
| `--provider <provider>` | `codex` \| `claude` | 无 |
