...
`im` 块用于在运行时之后向 VeFaaS 部署一个机器人代理，把消息渠道接入运行时。凭据用 `${VAR}` 提供。目前支持飞书：
...
| `frontend.oauth2.client_secret` | 用户池客户端密钥。 |

## 网络

`network` 块控制公/私网访问与 VPC 接入。

| 字段 | 说明 |
| --- | --- |
| `enable_public_network` | 是否开启公网访问。 |
| `enable_private_network` | 是否开启私网访问。 |
| `vpc.vpc_id` | 使用 VPC 时必填。 |
| `vpc.subnet_ids` | 子网 ID 列表。 |
| `vpc.enable_shared_internet_access` | 是否开启共享公网访问。 |

## 日志投递

`tls` 块把运行时日志投递到火山引擎日志服务。启用需将 `enable_log` 设为 `true`。

| 字段 | 说明 |
| --- | --- |
| `enable_log` | 是否启用日志投递，启用该块必填。 |
| `tls_project_id` | 日志项目 ID。 |
| `tls_topic_id` | 日志主题 ID。 |
...
`tags` 为资源标签列表，每项形如 `{ key: team, value: ai }`。
