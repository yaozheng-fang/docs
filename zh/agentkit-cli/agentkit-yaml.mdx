...
`im` 块用于在运行时之后向 VeFaaS 部署一个机器人代理，把消息渠道接入运行时。凭据用 `${VAR}` 提供。目前支持飞书与企业微信，可任选其一或同时启用。
...
| `im.wecom.connection_mode` | 连接模式。`websocket` 为默认值，也是目前唯一支持的取值。 |
| `im.wecom.bot_id` | 企业微信机器人 ID。 |
| `im.wecom.secret` | 企业微信机器人密钥。 |
| `im.wecom.websocket_url` | 企业微信 WebSocket 接入地址，默认 `wss://openws.work.weixin.qq.com`。 |
| `im.wecom.send_thinking_message` | 智能体处理期间是否先发送一条“思考中”的过渡消息，默认 `true`。 |
