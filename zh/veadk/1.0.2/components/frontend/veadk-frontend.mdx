...
- **对话与调试**：与智能体多轮对话，展示思考过程、工具调用、Token 与用时；对话中还会渲染智能体返回的 [A2UI](/zh/veadk/1.0.2/components/frontend/a2ui) 富界面卡片。
...
一个自定义组件由两个部分组成，它们共享同一个 catalog id。后端部分见 [A2UI](/zh/veadk/1.0.2/components/frontend/a2ui#自定义组件)；前端部分如下。
