...
完整跨会话示例见[长期记忆 · 跨会话示例](/zh/veadk/1.0.1/components/memory/index#跨会话示例)。
...
<Warning>
`local` 后端仅将记忆保存在进程内存中，进程退出后数据即丢失，且无法跨进程共享。需要持久化或多实例共享，请改用 [VikingDB](/zh/veadk/1.0.1/components/memory/vikingdb)、[Mem0](/zh/veadk/1.0.1/components/memory/mem0)、[OpenSearch](/zh/veadk/1.0.1/components/memory/opensearch) 或 [Redis](/zh/veadk/1.0.1/components/memory/redis)。
</Warning>
