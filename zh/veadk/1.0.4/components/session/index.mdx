...
| :--- | :--- | :--- | :--- | :--- |
| `local` | 否（仅内存） | 无 | 本地调试、临时会话 | [本地内存](/zh/veadk/1.0.4/components/session/local) |
| `sqlite` | 是（本地文件） | 无 | 单机持久化 | [SQLite](/zh/veadk/1.0.4/components/session/sqlite) |
| `mysql` | 是 | MySQL | 分布式持久化 | [MySQL](/zh/veadk/1.0.4/components/session/mysql) |
| `postgresql` | 是 | PostgreSQL | 分布式持久化 | [PostgreSQL](/zh/veadk/1.0.4/components/session/postgresql) |
