...
| 后端 | 存储 | 依赖 | 适用场景 | 文档 |
| :--- | :--- | :--- | :--- | :--- |
| `local` | 内存向量索引 | `extensions` + embedding | 本地调试 | [本地内存](/zh/veadk/1.0.4/components/memory/local) |
| `viking` | VikingDB 记忆库（托管） | 火山引擎账号 | 生产推荐 | [VikingDB](/zh/veadk/1.0.4/components/memory/vikingdb) |
| `mem0` | Mem0 记忆库（托管） | Mem0 API Key | 生产推荐 | [Mem0](/zh/veadk/1.0.4/components/memory/mem0) |
| `openviking` | OpenViking 长期记忆 | OpenViking 服务与 API Key | 从完整会话提取用户实体、事件和偏好 | [OpenViking](/zh/veadk/1.0.4/components/memory/openviking) |
| `opensearch` | OpenSearch 向量库 | OpenSearch + embedding | 自建向量检索 | [OpenSearch](/zh/veadk/1.0.4/components/memory/opensearch) |
| `redis` | Redis 向量库 | Redis(RediSearch) + embedding | 自建向量检索 | [Redis](/zh/veadk/1.0.4/components/memory/redis) |
