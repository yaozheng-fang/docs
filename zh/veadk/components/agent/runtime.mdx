...
- `tools` 中的[函数工具](/zh/veadk/components/tools/custom-function)会作为可调用工具提供给模型；
- `tools` 中的 [MCPToolset](/zh/veadk/components/tools/custom-mcp)会完成工具发现与调用；
- 通过 `SkillToolset` 或 VeADK 旧入口加载的[技能](/zh/veadk/components/agent/skills)会交由 Codex 的技能机制使用。
