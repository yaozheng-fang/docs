---
title: "安全防护"
description: "基于回调的内容安全护栏，借助火山大模型应用防火墙审查各阶段内容，拦截不安全的输入与输出。"
---
...
<Note>
路线图：Agent 级回调。`content_safety` 还提供 `before_agent_callback` 与 `after_agent_callback`，但目前是占位实现（返回 `None`，不做任何检测），尚未启用。Agent 级的输入校验与上下文分析在规划中，暂时无需挂载这两个回调。
</Note>
