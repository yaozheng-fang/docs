...
description: "使用 OpenSearch 作为向量存储自建知识库检索，支持持久化与多实例共享"
---

`opensearch` 后端由 `OpensearchKnowledgeBackend` 实现，使用 OpenSearch 作为向量存储。知识文本先经 embedding 模型向量化，再通过 LlamaIndex 写入 OpenSearch，检索时按相似度召回。
...
`OpensearchKnowledgeBackend` 接受以下字段：

| 参数 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `index` | `str` | 无默认，由 `KnowledgeBase` 传入 | 知识库索引名，须符合 OpenSearch 命名规则。 |
