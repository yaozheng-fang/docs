---
title: "概述"
description: "通过全链路追踪记录智能体的执行过程，理解内部行为、定位问题并分析性能"
---
...
`OpentelemetryTracer` 是统一的追踪入口，位于 `veadk.tracing.telemetry.opentelemetry_tracer`。它持有一组导出器，在初始化时向全局 `TracerProvider` 注册各导出器的 span 处理器，并自动附加一个内存导出器用于本地留存与落盘。
...
| 导出器 | 类 | 目标平台 | 文档 |
| :--- | :--- | :--- | :--- |
| APMPlus | `APMPlusExporter` | 火山引擎 APMPlus，追踪与指标 | [APMPlus](/zh/veadk/components/observability/apmplus) |
| Cozeloop | `CozeloopExporter` | Cozeloop，链路观测与评测 | [Cozeloop](/zh/veadk/components/observability/cozeloop) |
| TLS | `TLSExporter` | 火山引擎日志服务 TLS | [TLS 日志服务](/zh/veadk/components/observability/tls) |
| 内存 | `InMemoryExporter` | 进程内存，本地调试与落盘 | [内存导出器](/zh/veadk/components/observability/inmemory) |
...
<Note>
`InMemoryExporter` 由 `OpentelemetryTracer` 自动管理，并被放在 span 处理器链首位以记录全部 span。请勿手动加入 `exporters` 列表，否则初始化会抛出 `ValueError`。
</Note>
...
导出的 JSON 包含每个 span 的名称、`span_id`、`trace_id`、起止时间、属性以及父 span 关系。更多细节参见[内存导出器](/zh/veadk/components/observability/inmemory)。
