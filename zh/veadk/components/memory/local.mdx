---
title: "使用本地内存"
description: "进程内的向量记忆后端，无需外部服务，适合本地开发与调试"
---
...
完整跨会话示例见[长期记忆 · 跨会话示例](/zh/veadk/components/memory/index#跨会话示例)。
...
<Warning>
`local` 后端仅将记忆保存在进程内存中，进程退出后数据即丢失，且无法跨进程共享。需要持久化或多实例共享，请改用 [VikingDB](/zh/veadk/components/memory/vikingdb)、[Mem0](/zh/veadk/components/memory/mem0)、[OpenSearch](/zh/veadk/components/memory/opensearch) 或 [Redis](/zh/veadk/components/memory/redis)。
</Warning>
