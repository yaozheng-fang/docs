...
description: "生产级 React 前端及其单进程启动、构建与认证方式"
---

VeADK 自带一个生产级 React 前端，用于渲染智能体经由 Google ADK API Server 流式返回的 [A2UI](/zh/veadk/components/frontend/a2ui)。`veadk frontend` 是一个自包含的启动器：在默认模式下，它以单个进程同时提供这套 React 界面与智能体 API，两者同源，因此无需单独部署后端，也无需处理跨域配置。
...
一个自定义组件由两个部分组成，它们共享同一个 catalog id。后端部分见 [A2UI](/zh/veadk/components/frontend/a2ui#自定义组件)；前端部分如下。
