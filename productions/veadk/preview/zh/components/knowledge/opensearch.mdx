...
en_link: "/productions/veadk/preview/en/components/knowledge/opensearch"
---
...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `backend` | `str \| BaseKnowledgebaseBackend` | `"local"` | 本页设为 `opensearch`；也可传后端实例 |
...
| `secret_token` | `DATABASE_OPENSEARCH_SECRET_TOKEN` | `str` | `""` | 配置字段存在，但此后端不使用该令牌 |
