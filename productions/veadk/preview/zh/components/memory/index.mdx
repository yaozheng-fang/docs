...
| `index` | `str` | `""` | 存储记忆所用的索引/集合名。不提供 backend_config 时回退到 `app_name`，再为空则用 `default_app`；提供配置字典时应设置非空 index |
...
上文记忆管理代码为同一异步流程中的片段：先为 APP_NAME、USER_ID、SESSION_ID 赋值并确认查询得到非空 Session，再调用保存；完整入口见跨会话示例。启用 all 策略会将思考、工具输入输出和媒体信息发送到存储服务，执行前确认这些内容适合保留
