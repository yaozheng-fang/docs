---
title: "代码沙箱"
en_link: "/productions/veadk/preview/en/components/tools/code-sandbox"
---
...
| `invoke_skill` | 以非阻塞方式创建技能任务，返回初始任务对象 |
| `poll_skill` | 获取技能任务的当前状态快照 |
...
<Note>
如果需要将完整的任务委派给远端沙箱，而非由协调智能体逐工具调用，可以使用 `AgentkitRemoteSandboxAgent` 子智能体。详见[远端沙箱子智能体](/productions/veadk/preview/zh/components/agent/remote-sandbox-agent)。
</Note>
...
使用 BytePlus 时设置 `CLOUD_PROVIDER=byteplus` 与 `BYTEPLUS_ACCESS_KEY`、`BYTEPLUS_SECRET_KEY`，STS 场景同时配置 `BYTEPLUS_SESSION_TOKEN`；火山引擎 STS 使用 `VOLCENGINE_SESSION_TOKEN`。Tool ID、区域和服务端点必须属于同一环境。创建普通 AIO 沙箱只能提供基础执行环境，`execute_skills` 还要求技能 A2A 服务可用，`coding` 与 `run_sandbox_agent` 要求目标目录存在可执行的 `agent.py` 工作流及其依赖

首次验证可先运行下文的 Python 版本检查，确认沙箱可达后再添加技能或业务数据

## 使用方法

```python title="examples/tools/run_code/agent.py" lines
import asyncio
...
| `timeout` | `int` | `300` | 命令执行的超时时间，单位为秒。取值范围为 1–300。 |
| `exec_dir` | `str` | `/tmp` | Shell 执行的工作目录。 |
| `env` | `dict[str, str] \| None` | `None` | Shell 执行使用的环境变量。 |
| `hard_timeout` | `int` | `300` | Shell 执行的硬超时时间，单位为秒。取值范围为 1–300。 |
...
```python title="run_code_bash.py" lines
import asyncio
...
```python title="sandbox_env.py" lines
import asyncio
...
<Note>
设置 `SKILL_SPACE_POLICY` 环境变量后，VeADK 会自动将其转发到 `execute_skills`、`invoke_skill`、`poll_skill` 和 `run_sandbox_agent` 创建的沙箱会话，使沙箱中的技能加载遵循与本地相同的筛选规则。无需通过 `extra_env_vars` 手动传递。详见[技能](/productions/veadk/preview/zh/components/agent/skills#技能空间策略)
</Note>

## 技能沙箱执行

`execute_skills` 通过 A2A 协议在技能沙箱中执行工作流。调用时发送非阻塞的 `message/send` 请求，随后按指数递增的间隔轮询任务状态，直到任务达到终态或超时。支持的最长执行时间为 1800 秒（30 分钟）。

调用 `execute_skills` 时，VeADK 会从凭证服务中读取当前请求的入站身份凭证（凭证键为 `inbound_auth`），并以 `inbound_auth` 请求头转发给技能沙箱，使沙箱中的工作流能够以原始用户身份执行。若当前请求未携带入站凭证，则不附加该请求头，不转发该用户身份；访问沙箱仍需有效的云服务凭证与权限。
...
| `tool_context` | `ToolContext` | — | 工具运行时上下文，由 VeADK 自动注入。 |
| `env_vars` | `Optional[dict[str, str]]` | `None` | 不再支持。传入非 `None` 值会报错。 |
| `timeout` | `int` | `1800` | 最大执行时间，单位为秒。取值范围为 1–1800。 |
...
```python title="execute_skills_timeout.py" lines
import asyncio
...
当配置了[同步工具线程池](/productions/veadk/preview/zh/components/agent/runtime#同步工具并行执行)且模型在同一轮中发起多个 `run_code` 调用时，VeADK 默认为每个调用分配独立的沙箱会话标识：在基础会话标识后附加该调用的函数调用标识，使各 `run_code` 调用在相互隔离的沙箱中并行执行，互不影响彼此的文件系统和运行环境。
...
| 环境变量 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `VEADK_RUN_CODE_ISOLATE_PARALLEL_CALLS` | `true` | 是否在并行调用时为每个 `run_code` 分配独立的沙箱会话标识。设为 `false` 时所有并行调用共享同一会话标识。接受 `1`、`true`、`yes`、`on`（不区分大小写）为开启，`0`、`false`、`no`、`off` 为关闭。 |

## 指定沙箱工作流的完整参数

`run_sandbox_agent` 不会为用户创建或部署 `agent.py`；目标沙箱需预先包含该文件和依赖

| 参数 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `workflow_prompt` | `str` | 必填 | 传给远端工作流的任务说明 |
| `tool_id` | `str` | 必填 | 明确指定的沙箱 Tool ID |
| `tool_context` | `ToolContext \| None` | `None` | 调用时必需，由工具运行时注入 |
| `skills` | `list[str] \| None` | `None` | 传给远端工作流的技能选择 |
| `timeout` | `int` | `900` | 远端工作流等待秒数，需在服务允许范围内 |
| `working_dir` | `str` | `/home/gem/veadk_skills` | 沙箱内工作目录，需包含 `agent.py` |
| `extra_env_vars` | `dict[str, str] \| None` | `None` | 本次执行的环境变量，按前文规则校验 |

`coding` 接收必填 `workflow_prompt: str`、运行时注入的 `tool_context=None` 与 `timeout: int = 900`，使用 `AGENTKIT_TOOL_ID_OPENCODE` 或默认 Tool ID 调用预配置工作流，不接受任意代码字符串作为程序直接执行

这些工具返回执行输出，不保证每次都返回单纯的成功文本；远端错误也可能以输出或响应对象返回。检查退出结果、标准错误与预期产物后再确认完成。技能等待在需要输入、需要授权或超时时可能结束，不能据此断言远端任务已取消
