...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `user_pool_name` | 必填 | VeIdentity 用户池名称（未提供 `user_pool_uid` 时使用） |
| `user_pool_uid` | `None` | 用户池 UID，优先于 `user_pool_name` |
| `client_name` | 必填 | 用户池客户端名称（未提供 `client_uid` 时使用） |
| `client_uid` | `None` | 客户端 UID，优先于 `client_name` |
| `client_secret` | `None` | 已有客户端密钥；与 `client_uid` 同时提供时跳过启动时的客户端查询 |
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `userinfo_url` | `None` | 用户信息端点 URL；配置后中间件会从该端点获取用户信息并存入会话 |
| `user_id_field` | `"sub"` | 从用户信息中提取用户标识的字段名 |
