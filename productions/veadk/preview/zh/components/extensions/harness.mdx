---
title: "Harness"
en_link: "/productions/veadk/preview/en/components/extensions/harness"
---
...
<Warning>
  两种模式不得混用。受管 Sidecar 模式启用时，`HarnessExtension.plugins()` 始终返回空列表，应用进程不挂载进程内增强插件。
</Warning>
...
```
先完成[模型配置](/productions/veadk/preview/zh/components/agent/model)。保存为 `app.py` 后执行 `python app.py`，应先输出插件名称，再输出智能体回复。回答校验是规则检查，不保证回答事实正确；压缩可能丢失细节，应按任务评估效果
...
组件与场景分别优先读取 `HARNESS_ENHANCE_COMPONENTS`、`HARNESS_ENHANCE_PROFILE`，再回退到不带 `ENHANCE` 的同名变量。字符上限、存储路径、压缩提供方和校验模式则优先读取不带 `ENHANCE` 的变量；不要对所有变量套用同一种优先级
...
<Note>
  通过 `HarnessExtension(profile="ops")` 使用默认组件时，会增加 `long_run_control`；显式提供组件列表或直接调用 `build_harness_plugins()` 时，应自行包含该组件。
</Note>
...
设置环境变量后，在构造智能体前创建 `HarnessExtension`，结束时调用 `close()` 或退出上下文管理器。检查 `sidecar_status_payload()`：`ok` 表示启动成功，`degraded` 表示降级，不能据此认定优化已生效。`HARNESS_SIDECAR_FAIL_OPEN=true` 默认允许失败后继续运行；业务要求必须启用 Sidecar 时应设为 `false`

独立 Harness 服务的 HTTP 接口、会话和部署步骤见[Harness 服务部署](/productions/veadk/preview/zh/deploy/harness)。Sidecar 绑定会影响模型和 MCP 请求的去向，接入前确认上游端点和凭据来源
