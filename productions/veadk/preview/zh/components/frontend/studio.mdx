...
A2A 节点通过 AgentKit 智能体中心发现并调用远程智能体。创建 A2A 节点时，从下拉框选择所属的 AgentKit 智能体中心；列表由服务端使用已配置的火山引擎凭证加载，凭证不会下发到浏览器。召回数量、地域和 OpenAPI 地址等额外配置可在「更多选项」中设置。通过智能体中心发现远程智能体的 A2A 节点既可以作为根智能体，也可以作为其他智能体的子智能体；作为子智能体时，其发现的远程智能体工具会挂载到父智能体上，无需为该子智能体单独指定名称或 A2A 地址。
