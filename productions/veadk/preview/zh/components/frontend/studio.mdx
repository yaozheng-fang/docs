...
- **Agent 信息**：从该 Runtime 部署的 Agent Server 实时读取元数据，包括名称、模型、描述、子智能体、工具、技能以及已挂载组件（知识库、记忆、提示词管理、样例库、运行处理器、链路追踪、工具集、插件等）及其后端类型。元数据还会通过 `searchSources` 标识智能体实际挂载的检索源，供智能搜索按需启用。该信息只包含用于展示的摘要，不会返回系统提示词、凭据、环境变量值或任意对象序列化内容。
