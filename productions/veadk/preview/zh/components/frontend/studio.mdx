...
Studio 每三分钟检查一次 `latest.json`，列出更新的版本及其变更说明和 Git SHA。确认升级后，Studio 会校验所选完整 Bundle，替换当前函数代码并重新发布原 Application。
...
| `--studio-update-bucket` | `str` | `veadk-studio` | 存放不可变 Studio 发布 Bundle 的 TOS Bucket；部署时写入函数环境变量 `VEADK_STUDIO_UPDATE_BUCKET`。也可通过 `VEADK_STUDIO_UPDATE_BUCKET` 设置。 |
