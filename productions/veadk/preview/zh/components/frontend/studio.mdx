...
`--region` 指定 Studio 部署地域，默认为 `cn-beijing`，也支持 `cn-shanghai`。部署时会先查询部署地域的 VeIdentity 用户池客户端，未命中时再跨地域查询北京、上海两个地域；跨地域命中时会显示警告并继续。`--project` 指定 VeFaaS 函数所属项目，默认为 `default`。
...
| `--region` | `cn-beijing` \| `cn-shanghai` | `cn-beijing` | Studio 部署地域，决定 VeFaaS、API Gateway 和相关资源所在区域；部署时优先查询该地域的 VeIdentity 用户池客户端，未命中时跨地域查询北京、上海。 |
