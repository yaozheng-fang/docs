...
`--region` 指定 Studio 的部署地域，可选 `cn-beijing`（默认）或 `cn-shanghai`。部署时先在部署地域查询 VeIdentity 用户池，未命中时再跨地域查询北京、上海的用户池；命中非部署地域的用户池时会输出 warning 并继续，部署的 Studio 会使用该用户池所在地域与 VeIdentity 通信。`--project` 指定 VeFaaS 函数所属的火山引擎项目，默认为 `default`。
...
| `--region` | `cn-beijing \| cn-shanghai` | `cn-beijing` | Studio 部署地域，同时驱动跨地域 VeIdentity 用户池查询。 |
| `--project` | `str` | `default` | VeFaaS 函数所属的火山引擎项目。 |
