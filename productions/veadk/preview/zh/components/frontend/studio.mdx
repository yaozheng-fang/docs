...
- **Skill Hub**：从内置技能中心选择预置技能。
- **本地上传**：拖入文件夹或选择 ZIP 包。每个技能目录需包含 `SKILL.md` 文件；Studio 检查 `SKILL.md` 是否存在、文件数量、大小和路径安全，`SKILL.md` frontmatter 与技能格式的完整校验由 ADK 在加载时执行。生成项目中的技能目录名取自 `SKILL.md` 的 `name` 字段或上传目录名。
- **AgentKit SkillSpace**：按地域和项目浏览技能空间中的技能并选择版本。
