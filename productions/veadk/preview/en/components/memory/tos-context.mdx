...
zh_link: "/productions/veadk/preview/zh/components/memory/tos-context"
---
...
| `user_id` | `str` | `""` | Deprecated; use user_id from Session or search arguments |
...
This backend uses `VOLCENGINE_*` credential fields. Global BytePlus configuration can map AK/SK, but does not thereby switch the ContextBucket endpoint; match the endpoint, region, and session token to the target service. Availability of ordinary TOS on BytePlus does not establish ContextBucket availability
