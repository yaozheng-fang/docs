...
also supports `cn-shanghai`. Deployment first queries the user-pool client in
the deployment region; if it is not found, it falls back to searching the
Beijing and Shanghai Identity regions and continues with the region where the
client is found, emitting a warning on a cross-region match. `--project` selects
the Volcengine project for the VeFaaS function and defaults to `default`.
...
| `--region` | `cn-beijing` \| `cn-shanghai` | `cn-beijing` | Studio deployment region. Determines the region for VeFaaS, API Gateway, and related resources. The user-pool client is first looked up in this region; if not found, Beijing and Shanghai Identity regions are searched. |
...
| `--veadk-version` | `str` | Latest release | `veadk-python` version installed in VeFaaS. Set it to `1.0.5` to reproduce this version. |
