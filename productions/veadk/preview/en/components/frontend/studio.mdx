...
Deployment also automatically creates or reuses a snapshot-enabled Studio Sandbox Tool for [code projects](#studio-sandbox-code-projects). The Tool defaults to 8 CPU cores and 16 GB of memory, using the `studio-sandbox` image corresponding to the deployment region:
...
| `STUDIO_WORKSPACE_TOOL_ID` | `str` | Auto-create | Private Tool ID for Studio Sandbox code projects. Automatically created or reused during deployment; auto-provisioned during updates. |
