...
<Note>
  Session-capability management is an unreleased Preview feature available only when running or deploying Studio from the current VeADK source. Run it locally from source, or deploy with `--from-source`; without it, deployments use the latest released version and do not include this feature.
</Note>
