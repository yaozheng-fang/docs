...
A2A nodes discover and invoke remote agents through an AgentKit agent center. When creating an A2A node, select the agent center from a dropdown; the list is loaded server-side using the configured Volcengine credentials, and the credentials are never sent to the browser. Additional settings such as top K, region, and the OpenAPI endpoint are available under "More options". A2A nodes that use an agent center to discover remote agents can serve as either the root agent or a sub-agent of another agent. When used as a sub-agent, the discovered remote-agent tools are attached to the parent agent, and the sub-agent does not need a separate name or A2A URL.
