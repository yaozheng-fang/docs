...
Studio capabilities such as video creation and automatic evaluation require persistent object storage to save reference assets, generated results, and optimization snapshots. Studio uses TOS as the persistent storage backend.
...
<Note>
  Model selection in custom creation is filtered by cloud provider. BytePlus mode shows only BytePlus-compatible models, while Volcengine mode shows all models. Switching cloud providers automatically migrates the current model and API endpoint to the corresponding provider defaults.
</Note>
...
Studio calls the `doubao-seed-2-0-lite-260428` model (BytePlus uses `seed-2-0-lite-260228`) to produce a validated, complete agent-configuration draft that is loaded onto the canvas. Generation consumes tokens.
...
- Every generated LLM agent uses the Volcengine default model `doubao-seed-2-1-pro-260628` (BytePlus uses `seed-2-0-lite-260228`).
...
4. Track progress in the video task dialog. When generation completes, preview and download the resulting video.
...
<Note>
  Video generation runs asynchronously. The dialog shows the real-time status of both the enhancement and generation stages. When a stage fails, you can retry from the failed stage without re-entering the prompt.
</Note>
...
<Note>
  When uploading a ZIP, Studio checks that the archive contains `SKILL.md` and validates file count and path safety. Full frontmatter and skill format validation is performed by ADK at load time. A validation function is available to pre-check ZIP contents before upload.
</Note>
...
The model list is read from the DevEnv Tool configuration and filtered by the current cloud provider; a model ID can also be entered manually. BytePlus mode shows only BytePlus-compatible models.
...
<Steps>
  ...
  <Step title="Download or publish">
    After generation completes, download the skill as a ZIP or publish it directly to the current skill space. New skills are published as new skills; when optimizing an existing skill, you can choose to overwrite the original.
  </Step>
</Steps>
...
The model, endpoint, and candidate regions for sandbox Tools differ by cloud provider: Volcengine uses the `doubao-seed-2-1-pro-260628` model with `https://ark.cn-beijing.volces.com/api/v3`, and candidate regions `cn-beijing` and `cn-shanghai`; BytePlus uses the `seed-2-0-lite-260228` model with `https://ark.ap-southeast.bytepluses.com/api/v3`, and candidate region `ap-southeast-1`.
...
After signing in, the account menu at the bottom of the sidebar offers a "System info" entry. Selecting it opens the system information page, which displays the Studio version, storage, sandbox information, and user pools. This page is available only to the `admin` role, and all resource identifiers are read-only.
...
Displays the TOS address used by Studio persistent storage. When `VEADK_STUDIO_TOS_BUCKET` and `VEADK_STUDIO_TOS_REGION` are configured, the bucket access address is shown, for example `veadk-studio-<account ID>.tos-cn-beijing.volces.com`. When not configured, "Not configured" is displayed.

### Sandbox information

Lists the sandbox Tools configured in Studio and their IDs, including Codex Sandbox (`SANDBOX_CHAT_CODEX`), OpenClaw Sandbox (`SANDBOX_CHAT_OPENCLAW`), Hermes Sandbox (`SANDBOX_CHAT_HERMES`), and Dev Sandbox (`SANDBOX_DEV`). Tools without a configured ID show "Not configured".

### User pools

Lists the VeIdentity user pools associated with the current Studio, showing name, UID, domain, and region. When running locally without Volcengine credentials configured, no user pools are displayed.
