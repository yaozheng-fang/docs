...
`--region` selects the Studio deployment region, either `cn-beijing` (default) or `cn-shanghai`. During deployment, the VeIdentity user pool is first looked up in the deployment region; on a miss, the Beijing and Shanghai Identity regions are searched. When the user pool is found outside the deployment region, a warning is printed and deployment continues; the deployed Studio communicates with VeIdentity using that user pool's region. `--project` selects the Volcengine project for the VeFaaS function and defaults to `default`.
...
| `--region` | `cn-beijing \| cn-shanghai` | `cn-beijing` | Studio deployment region; also drives the cross-region VeIdentity user-pool lookup. |
| `--project` | `str` | `default` | Volcengine project for the VeFaaS function. |
