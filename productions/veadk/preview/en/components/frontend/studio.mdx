...
<Note>
  Orchestrator agents (sequential, parallel, loop) only schedule sub-agents and do not carry a model, instruction, or other configuration. Before publishing or debugging, an orchestrator agent must have at least one sub-agent; otherwise Studio reports the missing sub-agent in the type section.
</Note>
...
When you delete an agent, the deleted entry is removed from the list immediately. If the deleted agent is the one currently in use, Studio clears that selection and returns to the agent list.
