...
zh_link: "/productions/veadk/preview/zh/components/frontend/studio"
---
...
| Task | Guide |
| --- | --- |
| Start and connect to cloud agents | [Start and connect to cloud agents](#start-locally) |
| Create or import a project | [Create or import a project](#create-an-agent) |
| Migrate an existing application | [Migrate an existing application](#migrate-an-existing-project) |
| Configure storage | [Configure storage](#studio-persistent-storage) |
| Deploy Studio itself | [Deploy Studio itself](#deploy-to-vefaas) |
| Upgrade a deployed workbench | [Upgrade a deployed workbench](#update-a-deployed-studio) |

## Version and availability

This page covers released VeADK 1.1.13 features and the next Preview. API key status and model-permission filtering, the migration home layout, session artifacts, GitLab MR review, MCP credential editing, and new Studio deployment resource defaults were added after 1.1.13. A regular PyPI installation does not include those additions

Use the following commands in a separate virtual environment to reproduce this Preview. The source revision is pinned for consistent UI and behavior. Release-package users can proceed to local startup

```bash
git clone https://github.com/volcengine/veadk-python.git veadk-preview
cd veadk-preview
git checkout adcdfdcc6a5a213b249a8caad435b939c01df7f6
python -m venv .venv
...
Python and Node.js are required; see [Installation](/productions/veadk/preview/en/get-started/installation). When starting from this source directory, pass `--frontend-dir ./frontend/dist` to use the UI you just built
...
veadk studio --provider volcengine --open
```
...
Complete BytePlus startup example:

```bash
export BYTEPLUS_ACCESS_KEY="your-access-key"
export BYTEPLUS_SECRET_KEY="your-secret-key"
veadk studio --provider byteplus --open
```

To debug local agents, use `veadk studio --dev --agents-dir ./agents --open`; see [VeADK Web](/productions/veadk/preview/en/components/frontend/veadk-web#start) for the layout. `--dev` selects the local list; `--agents-dir` only supplies its directory

After startup, confirm that the home page opens, then select an available agent and send a message to verify the model and Runtime connection. If the cloud list is empty, check provider, region, and permissions before changing local agent directories
...
When configuring an LLM agent, you can browse the Ark models activated under the current account in the model selector and choose a target model. The model list is fetched by the Studio server from Ark and includes only LLM and VLM models that support agent calls, showing the model name, display name, vendor, and activation status. The list is cached server-side; use the refresh button to retrieve the latest status.
...
<Note>
  For the full parameter reference and usage restrictions for model fallbacks, see [Model](/productions/veadk/preview/en/components/agent/model#configure-cross-provider-fallback-models).
</Note>
...
| Environment variable | Description |
| :--- | :--- |
| `FALLBACK_MODEL_<agent_name>_<index>_API_KEY` | API key for a cross-provider fallback model, numbered from 1 in fallback order. |
...
| Min instances | `1` | Minimum number of Runtime instances; can be set to `0` to scale to zero. |
...
| Auto-create evaluation sets | Disabled | Automatically creates Good Case and Bad Case evaluation sets in Studio persistent storage for the agent after deployment succeeds. Disabling skips this step. If creation fails, a warning is shown in the deployment result without affecting the deployed Runtime. |

The minimum instance count must be a non-negative integer and the maximum a positive integer; the minimum cannot exceed the maximum. When the agent's short-term memory backend is `local` (in-memory) or unconfigured, Studio defaults the maximum instance count to 1 and warns that multiple instances, process restarts, or rolling updates can cause session loss; a database-backed short-term memory store is recommended. When creating a new Runtime, the minimum and maximum instance limits are set during creation; after deployment, Studio waits for the configured number of instances to become ready before reporting success, preventing the first conversation from landing on a temporary startup instance.
...
| Base environment | AIO Sandbox, Codex Sandbox, Ubuntu | The base image type, determining whether the image includes the Sandbox Shell or Codex task delegation capability. See [Base environment types](#base-environment-types). |
| Operating system | Ubuntu 22.04, Ubuntu 24.04 | The base image operating system. Fixed to Ubuntu 22.04 when AIO Sandbox or Codex Sandbox is selected. |
| Python version | Python 3.10, Python 3.12 | The Python version in the image. Fixed to Python 3.12 when AIO Sandbox or Codex Sandbox is selected. |
...
| Codex Sandbox | Built on the Codex Sandbox image, exposing task delegation through its Codex App Server instead of the generic Sandbox Shell tool. When selected, the operating system is fixed to Ubuntu 22.04 and Python is fixed to 3.12. After the image build completes, Studio automatically creates an associated AgentKit Sandbox Tool so the environment can be mounted to conversations for delegating complete tasks to Codex. |
...
| Session TTL | 1 hour (2 hours when evaluation is enabled) | Lifetime of the Dev Sandbox Session; artifacts in the temporary migration environment become unavailable after expiry. |
...
| Delivery content | — | When downloading a ZIP or deploying a migration artifact, the delivery includes configuration files from the project's `.agentkit/` directory (such as `agentkit.yaml` and `Dockerfile`) but excludes build artifacts and migration session state within that directory. |
...
<Note>
  Migration effect evaluation requires the administrator to configure Studio persistent storage (`VEADK_STUDIO_TOS_BUCKET` and `VEADK_STUDIO_TOS_REGION`). When not configured, the evaluation toggle shows "Migration effect evaluation is unavailable in this environment" while migration itself is unaffected. See <a href="#studio-persistent-storage">Studio persistent storage</a> for configuration.
</Note>

#### Enabling evaluation

When creating a migration task, the "Migration effect evaluation" toggle in the migration workspace is off by default. When enabled, the workspace shows two tabs — "Migration" and "Evaluation" — letting you configure evaluation cases while migration is in progress.

Enabling evaluation extends the Dev Sandbox Session TTL from 1 hour to 2 hours to accommodate temporary Runtime deployment, case execution, and evaluation analysis.

#### Configuring evaluation cases

Before migration completes, fill in evaluation cases on the "Evaluation" tab. Cases are locked once upload starts and cannot be modified.

Two input methods are supported:

| Input method | Description |
| :--- | :--- |
| Manual | Add cases one by one, entering user input, expected outcome, and requirements. |
| Paste multiple | Enter one case per line as user input for quick bulk addition. |

Each evaluation case includes:

| Field | Required | Type | Limit | Description |
| :--- | :--- | :--- | :--- | :--- |
| User input | Yes | `str` | Conversation text per case up to 32 KiB; at most 20 messages per case | The real user request the migrated agent should handle. |
| Expected outcome | No | `str` | Up to 16 KiB | Describe what the agent should accomplish; exact wording is not required. |
| Requirements | No | `list[str]` | At most 20 items; each up to 2 KiB | Specific conditions the agent output must satisfy. |
...
Evaluation dimensions determine which aspects of behavioral consistency are checked in the report. Two modes are supported:

| Mode | Default dimensions | Description |
| :--- | :--- | :--- |
| Standard evaluation | Semantic fidelity, output contract, workflow/tool fidelity | Suitable for most migrations; dimensions cannot be changed. |
| Custom dimensions | User selects from the dimensions below | Choose one or more dimensions based on business risk. |

Available dimensions:

| Dimension | Description |
| :--- | :--- |
| Semantic fidelity | Checks whether intent, conclusions, and key facts remain consistent. |
| Output contract | Checks required fields, structure, language, and formatting constraints. |
| Workflow and tool fidelity | Checks observable workflow branches and tool-driven behavior. |
| Context and memory fidelity | Checks supported multi-turn context and memory behavior. |
| Boundary and error fidelity | Checks invalid input, missing information, and dependency failures. |
| Safety and refusal fidelity | Checks existing authorization, refusal, and sensitive-data boundaries. |
...
After evaluation completes, the "Evaluation" tab shows a report summary including overall fidelity score (0–100), evidence coverage, execution success rate, N/A count, lowest-scoring cases, execution issues, and critical evidence. The report does not issue a pass/fail verdict; it presents quantitative scores and difference evidence. Click "View report" to preview the full HTML report in a side drawer, with download support.

The report and the locked evaluation dataset are stored as immutable assets in the private Studio TOS bucket, accessible only to the task owner.

| Report field | Description |
| :--- | :--- |
| Overall fidelity | 0–100 score; N/A when evidence is insufficient. |
| Evidence coverage | Ratio of dimensions with evidence to total dimensions. |
| Execution success | Ratio of successfully completed cases to total cases. |
| N/A count | Number of dimensions with insufficient evidence, excluded from scores. |
| Lowest-scoring cases | Cases with the lowest scores and their difference evidence. |
| Execution issues | Cases that failed to execute and their error details. |
| Critical evidence | Evidence entries with Critical severity. |
| Evaluation limitations | Known limitations affecting the evaluation conclusion. |
...
| State | Description |
| :--- | :--- |
| Evaluation is off | Migration effect evaluation is not enabled. |
| Waiting for evaluation cases | Evaluation is enabled but cases have not been saved yet. |
| Evaluation starts automatically after migration | Cases are saved; waiting for migration to complete. |
| Preparing the evaluation environment | Validating the migration artifact and evaluation cases. |
| Runtime environment variables are required | The temporary Runtime needs environment variables to start. |
| Deploying a temporary Runtime | Deploying the temporary Runtime for evaluation. |
| Running evaluation cases | Executing evaluation cases and capturing output. |
| Running evaluation analysis | Scoring per dimension and generating evidence. |
| Aggregating evaluation results | Summarizing scores and evidence, generating the report. |
| Evaluation completed | The report is ready for viewing and download. |
| Evaluation incomplete | An error occurred during evaluation. |
| Evaluation requires attention | The migration artifact is not deployable or session time is insufficient; resolve and retry. |
| Evaluation cancelled | The user manually cancelled evaluation, or migration did not produce an evaluable artifact. |
...
<Note>
  An agent with a pending review application must be withdrawn first, and a published agent must be unpublished first, before it can be modified or deleted. See [Agent publication review](#agent-publication-review).
</Note>
...
<Note>
  Skill customization shares the same generation and optimization flow, Dev Sandbox session management, candidate comparison, and publishing flow as the [Skill Center](#skills). The workspace acts only as a quick entry point. Browsing AgentKit SkillSpace is performed by the Studio server using its own configured Volcengine credentials; the browser never sees credentials.
</Note>
...
| Environment variable | Default | Description |
| :--- | :--- | :--- |
| `VEADK_GITHUB_APP_ID` | — | The GitHub App ID. |
| `VEADK_GITHUB_APP_SLUG` | — | The GitHub App slug, used to generate the install URL. |
| `VEADK_GITHUB_APP_WEBHOOK_SECRET` | — | The GitHub App webhook secret; must match the value in the GitHub App settings. |
| `VEADK_GITHUB_APP_PRIVATE_KEY` | — | The GitHub App private key content, with newlines represented as `\n`. |
| `VEADK_GITHUB_APP_PRIVATE_KEY_B64` | — | Base64-encoded GitHub App private key. Mutually exclusive with `VEADK_GITHUB_APP_PRIVATE_KEY` and `VEADK_GITHUB_APP_PRIVATE_KEY_PATH`; Base64 encoding takes priority. |
| `VEADK_GITHUB_APP_PRIVATE_KEY_PATH` | — | Path to the GitHub App private key file. Used when neither of the above is set. |
| `VEADK_GITHUB_APP_REVIEW_OWNER_ID` | `github-app` | The owner identifier for review tasks. |
| `VEADK_GITHUB_APP_REVIEW_CREATOR` | `GitHub App` | The display name for review tasks. |
...
| Environment variable | Default | Description |
| :--- | :--- | :--- |
| `VEADK_GITLAB_BASE_URL` | `https://gitlab.com` | GitLab instance URL; must be an HTTPS URL without path parameters. Specify the actual URL for self-hosted GitLab instances. |
| `VEADK_GITLAB_WEBHOOK_SECRET` | — | GitLab webhook verification token; must match the value configured in the project webhook. |
| `VEADK_GITLAB_OAUTH_CLIENT_ID` | — | GitLab OAuth application Client ID. |
| `VEADK_GITLAB_OAUTH_CLIENT_SECRET` | — | GitLab OAuth application Client Secret. |
| `VEADK_GITLAB_OAUTH_REDIRECT_URI` | — | OAuth redirect URI, pointing to the Studio `/web/gitlab/oauth/callback` path. |
| `VEADK_GITLAB_GROUP_ID_OR_PATH` | — | GitLab Group ID or path to restrict the project list scope. When unset, all projects where the current user is a member are listed; when set, only projects within that group (including subgroups) are listed. |
| `VEADK_STUDIO_PUBLIC_BASE_URL` | — | Public base URL of Studio, used to generate the GitLab webhook callback URL. When unset, project webhooks cannot be created automatically. |
...
| Submit for review | Submit a selected version of a personal skill for review. Studio copies the version snapshot into an independent skill in the review space, preserving the original name and recording the submitter. Duplicate submissions of the same source version are rejected while pending or approved. |
...
<Steps>
  ...
  <Step title="Shared skill management">
...
Submitted skill versions are automatically assessed using AI. Assessment uses the same model as automatic agent creation and scores through the Ark structured-output API.

| Dimension | Weight | Description |
| :--- | :--- | :--- |
| Safety | 40% | Evaluates whether the skill poses security risks. |
| Usability | 25% | Evaluates whether the skill is easy to understand and use. |
| Completeness | 15% | Evaluates whether the skill's functionality is fully covered. |
| Reliability | 10% | Evaluates the skill's stability across different scenarios. |
| Maintainability | 10% | Evaluates the maintainability of the skill's code and documentation. |

Studio calculates the weighted total and includes per-dimension scoring rationale, risks, and suggestions. Assessment reads only the fixed submitted snapshot without running its code or tools. Each dimension receives a score and rationale; when file coverage is incomplete, the safety, completeness, and total scores may be omitted.
...
The Review Center is located in the sidebar's Administration group, alongside User Management. It is visible only to administrators; the group is hidden entirely when no entries are visible.

Administrators can switch between skill and agent review requests in the Review Center, with region, status filters, and search.

Skill review requests are loaded from the regional review space. Each request shows the AI assessment status and total score; the details view exposes per-dimension scoring rationale, risks, file coverage, model metadata, and JSON report download. Review requests support approval (with an optional comment) or return (with a required reason and optional comment), and display reviewer details and history.

Agent review requests are loaded from the Runtime list for the selected region, showing agents with a submitted review application. Each request shows the agent name, applicant, submission time, current version, and model; the details view shows the agent description, application notes, and configuration fingerprint verification result. Administrators can approve (with an optional comment), return (with a required reason and optional comment), or publish directly. After a review action, the applicant can see the review status, reviewer, decision time, and comments on the agent card. See [Agent publication review](#agent-publication-review).
...
<Steps>
  ...
  <Step title="Enterprise visibility and unpublish">
...
| Status | Description |
| :--- | :--- |
| Pending | An application has been submitted and is awaiting administrator review. |
| Approved | The administrator has approved the application; the agent is visible to everyone. |
| Returned | The administrator returned the application; the submitter can resubmit after adjustments. |
| Withdrawn | The submitter withdrew the application before a decision was made. |
...
| `--super-admin` | `str \| None` | `None` | Initial super administrator email or UID when an Identity user pool is configured. Enables Identity role management; other users default to regular users. Also reads `VEADK_STUDIO_SUPER_ADMIN`. |
| `--admin` | `str \| None` | `None` | Comma-separated admin list (usernames or OAuth emails). Only effective locally without an Identity pool. Also reads `VEADK_STUDIO_ADMINS`. |
| `--developer` | `str \| None` | `None` | Comma-separated developer list (usernames or OAuth emails). Only effective locally without an Identity pool. Also reads `VEADK_STUDIO_DEVELOPERS`. |
...
| `--sandbox-chat-codex-tool-id` | `str` | `SANDBOX_CHAT_CODEX` | CodeEnv Tool ID for temporary chats |
| `--sandbox-chat-openclaw-tool-id` | `str` | `SANDBOX_CHAT_OPENCLAW` | ArkClawEnv Tool ID for OpenClaw chats |
| `--sandbox-chat-hermes-tool-id` | `str` | `SANDBOX_CHAT_HERMES` | HermesEnv Tool ID for Hermes chats |
| `--sandbox-chat-codex-snapshot-tool-id` | `str` | `SANDBOX_CHAT_CODEX_SNAPSHOT` | Snapshot-enabled CodeEnv Tool ID for persistent chats |
| `--sandbox-chat-openclaw-snapshot-tool-id` | `str` | `SANDBOX_CHAT_OPENCLAW_SNAPSHOT` | Snapshot-enabled Tool ID for persistent OpenClaw chats |
| `--sandbox-chat-hermes-snapshot-tool-id` | `str` | `SANDBOX_CHAT_HERMES_SNAPSHOT` | Snapshot-enabled Tool ID for persistent Hermes chats |
| `--help` | `bool` | — | Show help for the installed version |
...
Run from the installed and built Preview source root described above. `--from-source` builds and deploys that checkout. Prepare authorized Volcengine credentials first
...
| `--super-admin` | `str \| None` | `None` | Initial super administrator email or Identity UID. The user must already exist in the pool. Omit on first deployment to make everyone an admin; supplying it makes other users regular users by default. Later deployments preserve Identity roles. Also reads `VEADK_STUDIO_SUPER_ADMIN`. |
...
| `--studio-sandbox-tool-id` | `str \| None` | Auto-create | Tool ID for Studio Sandbox code projects; also read from `STUDIO_WORKSPACE_TOOL_ID`. When an existing persistent Tool is specified, it is reused without creating or modifying its configuration. |
| `STUDIO_WORKSPACE_IMAGE` | `str` | Region-dependent | Overrides the image reference used by the Studio Sandbox. Must be accessible in the selected region. |
...
veadk studio deploy \
  ...
  --region "cn-beijing" \
  --project "default" \
  --image "<vestack-registry>/<namespace>/veadk-studio:<tag>" \
  --public-domain "studio.example.com" \
  --vestack-openapi-url "https://openapi.example.com" \
  --vestack-agentkit-region "e70" \
  --vestack-apig-cluster-id "<cluster-id>" \
...
```text lines
✅ VeStack Studio Function deployed: http://studio.example.com:<port>
...
In VeStack deployment mode, Studio creates a dedicated AgentKit Tool for each sandbox session instead of sharing a single Tool. Independent Tools support disk persistence and are automatically cleaned up when the session is deleted.

When creating a session, you can specify the disk size (`diskGb` parameter), with a valid range of 5–100 GiB. The default value is determined by the deployment configuration.

| Sandbox type | Independent Tool type | Description |
| :--- | :--- | :--- |
| Codex (built-in agents) | `CodeEnv` | Creates an independent CodeEnv Tool per session, bound to an IAM Role, with disk persistence. |
| Hermes | `Station-Hermes` | Enabled when Hermes model parameters are configured at deploy time; creates an independent Hermes Tool per session and automatically starts the Hermes Dashboard. |

The Hermes independent Tool requires model parameters to be configured at deploy time (`--vestack-hermes-model-agent-name`, `--vestack-hermes-model-api-base`, `--vestack-hermes-model-api-key`, `--vestack-hermes-model-id`, and `--vestack-hermes-model-agent-name`). When not configured, the Hermes agent displays "Administrator has not configured the Hermes model or IAM Role."
...
| Environment variable | Description |
| :--- | :--- |
| `VEFAAS_OPENAPI_HOST` | VeFaaS OpenAPI host, overriding the default public-cloud address. |
| `VEFAAS_OPENAPI_SCHEME` | VeFaaS OpenAPI scheme (`http` or `https`). |
| `APIG_OPENAPI_HOST` | APIG OpenAPI host. |
| `APIG_OPENAPI_SCHEME` | APIG OpenAPI scheme. |
| `IAM_OPENAPI_HOST` | IAM OpenAPI host. |
| `IAM_OPENAPI_SCHEME` | IAM OpenAPI scheme. |
| `IDENTITY_OPENAPI_HOST` | Identity OpenAPI host. |
| `IDENTITY_OPENAPI_SCHEME` | Identity OpenAPI scheme. |
| `VOLCENGINE_OPENAPI_CA_BUNDLE` | Custom CA certificate path for private-certificate environments. When set, TLS verification is enabled. |
| `VOLCENGINE_OPENAPI_VERIFY_SSL` | Set to `false` to skip TLS certificate verification. Use only in controlled environments; prefer `VOLCENGINE_OPENAPI_CA_BUNDLE`. |

### VeStack deployment options

The following options take effect only when `--deploy-target vestack` is used; they supplement the [deployment options](#deployment-options).

| Option | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `--deploy-target` | `application` \| `vestack` | `application` | Deployment target. `application` uses the public-cloud VeFaaS Application; `vestack` uses a VeFaaS image function + APIG. Also read from `VEADK_STUDIO_DEPLOY_TARGET`. |
| `--image` | `str` | — | Prebuilt `linux/amd64` Studio image required for VeStack deployment. Also read from `VEADK_STUDIO_IMAGE`. |
| `--public-domain` | `str` | — | Dedicated APIG host for the VeStack Studio deployment. Also read from `VEADK_STUDIO_PUBLIC_DOMAIN`. |
| `--vestack-openapi-url` | `str` | — | VeStack control-plane URL; must include scheme and host, e.g. `https://openapi.example.com`. Also read from `VESTACK_OPENAPI_URL`. |
| `--vestack-agentkit-host` | `str` | `ops-top.ops-top.svc:8000` | AgentKit TOP host reachable from the VeFaaS data plane. Also read from `VESTACK_AGENTKIT_HOST`. |
| `--vestack-agentkit-region` | `str` | — | Delivered environment region ID, e.g. `e70`. Also read from `VESTACK_AGENTKIT_REGION`. |
| `--vestack-codex-port` | `int` (1–65535) | `8642` | Service port used by independent VeStack Codex Tools. Also read from `VEADK_STUDIO_CODEX_PORT`. |
| `--vestack-hermes-port` | `int` (1–65535) | `4500` | Dashboard port used by independent VeStack Hermes Tools. Also read from `VEADK_STUDIO_HERMES_PORT`. |
| `--vestack-hermes-model-agent-name` | `str` | — | AgentKit model endpoint name for independent VeStack Hermes Tools. Also read from `VEADK_STUDIO_HERMES_MODEL_AGENT_NAME`. |
| `--vestack-hermes-model-api-base` | `str` | — | Model API base injected into independent VeStack Hermes Tools. Also read from `VEADK_STUDIO_HERMES_MODEL_API_BASE`. |
| `--vestack-hermes-model-api-key` | `str` | — | Model API key injected into independent VeStack Hermes Tools; prefer using environment variables. Also read from `VEADK_STUDIO_HERMES_MODEL_API_KEY`. |
| `--vestack-hermes-model-id` | `str` | — | Model endpoint ID injected into independent VeStack Hermes Tools. Also read from `VEADK_STUDIO_HERMES_MODEL_ID`. |
| `--vestack-apig-cluster-id` | `str` | — | VeStack cluster ID used by the APIG ClusterSpec. Also read from `VESTACK_APIG_CLUSTER_ID`. |
| `--vestack-apig-namespace` | `str` | `veadk-studio-apig` | Existing Kubernetes namespace where APIG installs the gateway. Also read from `VESTACK_APIG_NAMESPACE`. |
| `--vestack-apig-cluster-name` | `str` | `aio` | VeStack APIG cluster display name. Also read from `VESTACK_APIG_CLUSTER_NAME`. |
| `--vestack-insecure-skip-tls-verify` | Boolean flag | `false` | Allow a private or self-signed OpenAPI certificate. Also read from `VESTACK_INSECURE_SKIP_TLS_VERIFY`. |
...
On the conversation page, you can mount built AIO Sandbox or Codex Sandbox runtime environments to the current session. Once mounted, the agent gains the following tools for executing shell commands or delegating tasks inside the mounted environments:
...
| `execute_in_sandbox` | Executes a non-interactive shell command inside a non-Codex mounted environment, including installed CLI tools. Codex Sandbox environments do not support this tool; use `delegate_to_codex_sandbox` instead. Supports setting the working directory, timeout, and waiting for a still-running command to return its result. |
| `delegate_to_codex_sandbox` | Delegates a complete coding, review, authoring, or engineering task to Codex inside a mounted Codex Sandbox environment. Applies to environments whose base environment is Codex Sandbox. Pass a self-contained task description (including the expected outcome, constraints, relevant context, and verification requirements); Codex inspects the environment and invokes its installed CLIs end to end. Call at most once per conversation turn; when it reports busy or timeout, do not retry automatically — surface that state to the user. |
...
The Studio BFF dynamic tools include a `branch_compare` tool. When the user asks to compare two styles, two approaches, or two creative directions, the agent can call this tool to generate two directly comparable branches in parallel and present the results as an inline card in the conversation.

The tool accepts the following input parameters:

| Parameter | Type | Description |
| :--- | :--- | :--- |
| `prompt` | `string` | The shared question or task both branches answer, 1–12,000 characters. |
| `branches` | `array` | Exactly two branch objects. |
| `branches[].label` | `string` | Branch name, 1–10 characters. |
| `branches[].instruction` | `string` | Branch-specific generation requirements, 1–4,000 characters. |

The two branches are generated independently and in parallel. Each branch uses a dedicated agent in a dedicated session to produce Markdown text. Progress is streamed back incrementally during generation, and the final text is returned upon completion. The card displays both branches as tabs; users can switch between them and use the **Continue this direction** button to fill the selected branch into the input box for further conversation. If one branch fails, its error is shown without affecting the other branch.

The model used for branch comparison defaults to `doubao-seed-2-0-lite-260428` and can be overridden with the `VEADK_STUDIO_BRANCH_MODEL` environment variable. The tool timeout is 120 seconds.
...
The user and session identifiers are determined by the authenticated tool context — the agent cannot select the owner, session, bucket, or credentials through its parameters. Files are saved to `artifacts/{user_id}/{session_id}/{relative_file_path}`. Saving the same path replaces the existing file for the current session.

The tool accepts the following input parameters:

| Parameter | Type | Description |
| :--- | :--- | :--- |
| `path` | `string` | A relative file path within the session artifact directory, 1–1024 characters. |
| `content` | `string` | The complete UTF-8 text content of the file, up to 1 MiB. |

On success, the tool returns the artifact path, file name, MIME type, size, and SHA-256 digest. Supported formats include HTML, SVG, Markdown, JSON, CSV, and code in UTF-8 text, with a maximum file size of 1 MiB per file.

#### Browsing and previewing artifacts

A "Session artifacts" button above the chat composer opens an artifact browser panel. The panel lists saved artifacts for the current session as a file tree, supporting refresh and pagination; the list auto-refreshes after a reply completes. Switching sessions closes the previous preview.

Previewable formats include HTML, Markdown, images, JSON, and plain text. HTML previews render in a scriptless sandbox that allows only inline styles and up to 32 authenticated, same-session relative images; external resources and scripts are disabled. Other file formats can be downloaded directly.
...
The session artifacts feature depends on Studio persistent storage, which requires the `VEADK_STUDIO_TOS_BUCKET` and `VEADK_STUDIO_TOS_REGION` environment variables. See [Studio persistent storage](#studio-persistent-storage). The Studio execution identity needs read, write, and list access to the bucket. Reads and writes use the Studio bucket's region on both Volcengine and BytePlus.

When Studio storage is configured, artifact previews always use that bucket, even if the Runtime has an unrelated TOS mount. When Studio storage is not configured, the reader retains support for an existing Runtime artifact mount, but the write tool is unavailable.
...
The Studio sidebar footer provides a "Try AgentKit CLI" entry. Clicking it opens a terminal dialog over the current page. The terminal runs inside an AgentKit Dev Sandbox and automatically executes `agentkit --help` and `agentkit --version` on startup, providing a quick way to inspect the available AgentKit CLI commands and the installed version.
...
| `--super-admin` | `str \| None` | `None` | Enable Identity user management and migrate the deployed admin/developer lists. Provide an existing user's email or UID as the initial super administrator. |
...
| `--byteplus-session-token` | `str \| None` | `BYTEPLUS_SESSION_TOKEN` | BytePlus STS temporary-credential session token, used only with `--provider byteplus`. |
| `--skip-cronjob-scheduler` | `bool` (flag) | Off | Update only the main Studio Application and leave the existing scheduled-task scheduler unchanged. Useful when you only need to update Studio code without affecting the scheduler service. |
...
On first deployment, omitting this option makes all signed-in users admins, including future users. Supplying it makes other users regular users by default. The default and individual roles are stored in Identity. Later deployments and updates preserve existing assignments.

Only super admins can see and use **User management** to list pool users and change roles. They inherit every admin capability. Studio protects the initial super admin from demotion. To assign the first super admin to an existing installation without changing other roles, run `veadk studio update --vefaas-app-name <app-name> --super-admin <email-or-uid>`.
...
| View and connect to Runtimes | All | All | Own and enterprise-visible | Own and enterprise-visible |
| Manage or delete Runtimes | All Studio-managed | All Studio-managed | Own only | Denied |
| Request agent publication | Allowed | Allowed | Allowed | Denied |
| Approve or publish agents directly | Allowed | Allowed | Denied | Denied |
| Unpublish agents | Allowed | Allowed | Own only | Denied |

The backend reads current Identity group membership for each authenticated request. Refreshing the page reflects role changes in permissions and the account badge.

In-app updates match the old `VEADK_STUDIO_ADMINS` and `VEADK_STUDIO_DEVELOPERS` entries to Identity users and preserve their roles. Only after migration succeeds are the old role environment values cleared. Unmatched or ambiguous entries and insufficient permissions stop migration and retain the old configuration. Empty legacy lists preserve the all-admin default; with explicit lists, unlisted users remain regular users. Existing admins are never automatically promoted to super admin.
...
After signing in, the account menu at the bottom of the sidebar offers a "System info" entry. Selecting it opens the system information page over the current page, which displays the Studio version, storage, sandbox information, and user pools. The back button in the top-left corner of the page returns to the page you were on before opening System info; the previous page is preserved. The TOS bucket, sandbox Tools, and user pools each provide a link that opens the corresponding page in the cloud console in a new tab. This page is available only to users with the admin or super admin role, and all resource identifiers are read-only; administrators can update sandbox Tools that have available image updates (see [Sandbox information](#sandbox-information)).
