...
<Note>
When adding skills from an AgentKit SkillSpace, if the cloud skill version exposes a full package, Studio downloads and includes all of its files (`SKILL.md`, scripts, references, and assets); only when the cloud version does not provide a full package does Studio use `SKILL.md` alone. There is no fixed limit on the number of skills a single agent can have.
</Note>
