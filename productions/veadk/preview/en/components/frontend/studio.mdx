...
- **Skill Hub**: Choose a prebuilt skill from the built-in skill center.
- **Local upload**: Drag in a folder or select a ZIP archive. Each skill directory must contain a `SKILL.md` file. Studio checks for `SKILL.md` presence, file count, size, and path safety; full validation of `SKILL.md` frontmatter and skill format is performed by ADK at load time. The skill folder name in the generated project is derived from the `SKILL.md` `name` field or the uploaded folder name.
- **AgentKit SkillSpace**: Browse skills in a skill space by region and project, then select a version.
