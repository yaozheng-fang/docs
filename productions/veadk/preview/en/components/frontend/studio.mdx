...
The agent creation flow supports three skill sources:

- **Skill Hub**: search the public Volcengine skill repository by keyword and add a skill.
- **Local upload**: drag a local skill directory or ZIP into Studio; the format is detected automatically.
- **AgentKit SkillSpace**: browse skill spaces visible to the current account and select skills from a space.
