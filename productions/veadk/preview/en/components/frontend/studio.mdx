...
| `--studio-sandbox-tool-id` | `str \| None` | Auto-create | Private Tool ID for Studio Sandbox code projects; also read from `STUDIO_WORKSPACE_TOOL_ID`. When an existing persistent Tool is specified, it is reused without creating or modifying its configuration. |
...
Lists the sandbox Tools configured in Studio and their IDs, including Codex Sandbox (`SANDBOX_CHAT_CODEX`), DeepSeek Harness Sandbox (`SANDBOX_CHAT_CODEX`), OpenClaw Sandbox (`SANDBOX_CHAT_OPENCLAW`), Hermes Sandbox (`SANDBOX_CHAT_HERMES`), Dev Sandbox (`SANDBOX_DEV`), and Studio Sandbox (`STUDIO_WORKSPACE_TOOL_ID`), shown in a fixed order. The Studio Sandbox is used for [code projects](#studio-sandbox-code-projects) and is a Private-type Tool with persistent snapshots enabled. DeepSeek Harness Sandbox shares the same AgentKit CodeEnv Tool as Codex Sandbox (`SANDBOX_CHAT_CODEX`), so the two display the same Tool ID. Configured Tool IDs provide a link to the corresponding Tool detail page in the cloud console. Tools without a configured ID show "Not configured".
