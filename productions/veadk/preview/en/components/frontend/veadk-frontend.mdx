...
zh_link: "/productions/veadk/preview/zh/components/frontend/veadk-frontend"
---
...
- **Create agents**: custom configuration is the available project-creation flow, producing a runnable VeADK project (`agent.py`, `requirements.txt`, and so on) that you can preview, edit, and download. See [Studio](/productions/veadk/preview/en/components/frontend/studio) for other creation flows and availability.
...
Complete [installation](/productions/veadk/preview/en/get-started/installation) and [model configuration](/productions/veadk/preview/en/components/agent/model). The distribution includes the UI, so Node.js and a frontend build are not required. Prepare an agent directory with `agent.py`, `__init__.py`, and an exported `root_agent`. See [A2UI project setup](/productions/veadk/preview/en/components/frontend/a2ui#project-setup) for the layout
...
Use this mode when editing frontend source. Start the backend from the VeADK source root, then start Vite in `frontend` from a separate terminal. Run `npm run build` for a production build and point `--frontend-dir` at the output

Use `--vite` to serve only the API and allow CORS from the Vite dev server (`http://localhost:5173`, falling back to `http://localhost:5174`). Add `--dev` to load local agents instead of cloud AgentKit runtimes.

```bash lines
veadk frontend --dev --vite --agents-dir ./agents   # API only, CORS for Vite
cd frontend
npm install
...
The `frontend` directory includes a standalone component preview page for browsing the shared component library. The preview runs without a backend service, organized by primitives, composites, layouts, node components, and AI APP components, with design tokens, light/dark theme switching, and parameter tables generated from TypeScript interfaces. This entry point is independent of the Studio production build and does not replace existing pages.

```bash lines
cd frontend
npm install
npm run dev:components
# open http://127.0.0.1:5186/components-preview/
```
...
| `--provider` | `volcengine` \| `byteplus` | Cloud provider for AgentKit services. Defaults to `AGENTKIT_CLOUD_PROVIDER` then `CLOUD_PROVIDER`, falling back to `volcengine` when neither is set; selecting `byteplus` reads credentials from `BYTEPLUS_ACCESS_KEY`, `BYTEPLUS_SECRET_KEY`, and the optional `BYTEPLUS_SESSION_TOKEN`, with runtime listings defaulting to the region in `BYTEPLUS_REGION` or `ap-southeast-1`. |
...
| `--site-title` | `VEADK_SITE_TITLE` | Site title, up to 16 characters |
| `--site-logo` | `VEADK_SITE_LOGO` | Local image or HTTP(S) logo URL |
| `--sandbox-chat-codex-tool-id` | `SANDBOX_CHAT_CODEX` | CodeEnv Tool ID for temporary chats |
| `--sandbox-chat-openclaw-tool-id` | `SANDBOX_CHAT_OPENCLAW` | ArkClawEnv Tool ID for OpenClaw chats |
| `--sandbox-chat-hermes-tool-id` | `SANDBOX_CHAT_HERMES` | HermesEnv Tool ID for Hermes chats |
| `--sandbox-chat-codex-snapshot-tool-id` | `SANDBOX_CHAT_CODEX_SNAPSHOT` | Snapshot-enabled CodeEnv Tool ID for persistent chats |
| `--sandbox-chat-openclaw-snapshot-tool-id` | `SANDBOX_CHAT_OPENCLAW_SNAPSHOT` | Snapshot-enabled Tool ID for persistent OpenClaw chats |
| `--sandbox-chat-hermes-snapshot-tool-id` | `SANDBOX_CHAT_HERMES_SNAPSHOT` | Snapshot-enabled Tool ID for persistent Hermes chats |
| `--super-admin` | `VEADK_STUDIO_SUPER_ADMIN` | Email or UID for initializing Identity-backed Studio roles |
| `--admin` | `VEADK_STUDIO_ADMINS` | Local administrator list; see Studio role configuration |
| `--developer` | `VEADK_STUDIO_DEVELOPERS` | Local developer list; see Studio role configuration |
| `--help` | — | Show help for the installed version |
...
```ts title="src/a2ui/components/RevenueChart/index.ts" lines
import { register } from "../../registry";
import { RevenueChart } from "./RevenueChart";
register("RevenueChart", RevenueChart);
```

```tsx title="src/a2ui/components/RevenueChart/RevenueChart.tsx" lines
