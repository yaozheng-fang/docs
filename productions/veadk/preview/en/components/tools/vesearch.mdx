...
zh_link: "/productions/veadk/preview/zh/components/tools/vesearch"
---
...
## Parameters and result checks

| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `query` | `str` | Required | Query text for the online Q&A service |

`TOOL_VESEARCH_ENDPOINT` is an existing agent ID, not an HTTP URL. The call returns a text summary, but failures may also return error text, so a string result alone does not establish success. The request timeout is 30 seconds. `CLOUD_PROVIDER=byteplus` does not switch this service.
...
This direct call uses only the Q&A service configuration. An outer reasoning model is additionally required when using the tool through `Runner`.
