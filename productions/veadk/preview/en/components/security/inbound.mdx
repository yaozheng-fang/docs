...
| --- | --- | --- |
| `user_pool_name` | required | VeIdentity user pool name (used if `user_pool_uid` is not set) |
| `user_pool_uid` | `None` | User pool UID, takes precedence over `user_pool_name` |
| `client_name` | required | User pool client name (used if `client_uid` is not set) |
| `client_uid` | `None` | Client UID, takes precedence over `client_name` |
| `client_secret` | `None` | Existing client secret; when paired with `client_uid`, skips the client lookup during startup |
