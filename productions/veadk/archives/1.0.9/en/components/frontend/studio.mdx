...
zh_link: "/productions/veadk/archives/1.0.9/zh/components/frontend/studio"
---
...
| Backend | When to use |
| :--- | :--- |
| [VikingDB Knowledge](/productions/veadk/archives/1.0.9/en/components/knowledge/viking) | The default. Uses the managed Volcengine knowledge base service for server-side splitting, embedding, and retrieval, without local embedding configuration. |
| [OpenSearch](/productions/veadk/archives/1.0.9/en/components/knowledge/opensearch) | Uses a self-managed or managed OpenSearch cluster with your embedding configuration. |
| [Context Search](/productions/veadk/archives/1.0.9/en/components/knowledge/context-search) | Uses the Volcengine Context Search service for retrieval. |

Studio does not offer the `local` backend because its creation flow cannot add documents to the in-process vector store. For local development, use the VeADK SDK to [configure a local knowledge base](/productions/veadk/archives/1.0.9/en/components/knowledge/local).
...
For the full parameters, shell execution, and credential requirements of `run_code`, see the [Code sandbox](/productions/veadk/archives/1.0.9/en/components/tools/code-sandbox).
