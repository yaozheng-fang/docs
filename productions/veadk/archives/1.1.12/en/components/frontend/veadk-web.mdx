---
title: "VeADK Web"
zh_link: "/productions/veadk/archives/1.1.12/zh/components/frontend/veadk-web"
---
...
At startup `veadk web` adjusts how the ADK dev server serializes the agent build graph. This endpoint powers the agent topology and configuration view in the debugging UI; the original serialization rendered the VeADK agent's model field as a complex structure containing runtime client objects, which the UI could not parse correctly.

After the adjustment, the model field in the build graph JSON is rendered as a `provider/model-name` string, runtime client objects are removed from the output, and the entire agent tree — including nested sub-agents — serializes correctly.
