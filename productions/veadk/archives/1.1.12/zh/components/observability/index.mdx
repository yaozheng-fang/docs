---
title: "概述"
en_link: "/productions/veadk/archives/1.1.12/en/components/observability"
---
...
运行日志使用 Python 标准库 `logging`，级别、格式与追踪关联方式见[应用日志](/productions/veadk/preview/zh/components/observability/logging)。
...
| `apmplus_managed_externally` | `bool` | — | 只读属性。初始化时是否检测到已存在的全局 `TracerProvider`。为 `True` 时表示 VeADK 复用外部 provider 而不再覆盖，并自动移除 `APMPlusExporter`。 |
...
此时，VeADK 会自动从导出器列表中移除 `APMPlusExporter`，因为已存在的 provider 被视为已负责 APMPlus 的链路数据上报。其他导出器（如 `CozeloopExporter`、`TLSExporter`）仍会被正常注册到该 provider。VeADK 的内存导出器也会正常附加。

<Note>
此行为同样适用于通过 `ENABLE_APMPLUS=true` 自动创建导出器的场景：如果全局 `TracerProvider` 已存在，自动创建的 `APMPlusExporter` 也会被移除。
</Note>

可通过 `apmplus_managed_externally` 属性判断当前追踪器是否检测到了外部 provider：

```python lines
tracer = OpentelemetryTracer(exporters=[APMPlusExporter()])
if tracer.apmplus_managed_externally:
    # VeADK 复用了已存在的全局 TracerProvider，APMPlusExporter 已被自动移除
    pass
```
