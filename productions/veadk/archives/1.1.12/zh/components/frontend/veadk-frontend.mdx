...
en_link: "/productions/veadk/archives/1.1.12/en/components/frontend/veadk-frontend"
---
...
`frontend` 目录包含一个独立的组件预览页面，用于浏览共享组件库中的组件实现。预览页面不依赖后端服务，按基础组件、复合组件、布局、节点组件和 AI APP 分组展示，包含设计 Token、明暗主题切换和从 TypeScript 接口生成的参数表。此入口独立于 Studio 生产构建，不替换现有业务页面。
