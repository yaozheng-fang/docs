---
title: "VeADK Web"
en_link: "/productions/veadk/archives/1.1.12/en/components/frontend/veadk-web"
---
...
`veadk web` 启动时会调整 ADK 开发服务器中智能体构建图的序列化行为。该端点用于在调试界面中展示智能体拓扑与配置，原序列化会将 VeADK 智能体的模型字段输出为包含运行时客户端对象的复杂结构，导致界面无法正确解析。

调整后，构建图 JSON 中的模型字段以「提供方/模型名称」的字符串形式呈现，运行时客户端对象不再出现在输出中，包含嵌套子智能体在内的整棵智能体树均可正常序列化。
