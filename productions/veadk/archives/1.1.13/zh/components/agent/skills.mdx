---
title: "技能"
en_link: "/productions/veadk/archives/1.1.13/en/components/agent/skills"
---
...
<Note>
`VeSkillRegistry` 加载技能时容忍 `SKILL.md` frontmatter 中的非标准字段（社区扩展），如 `allowed-tools`、`triggers`、`requires` 等。这些字段不会导致加载失败，而是保留在技能 frontmatter 的 `metadata` 中。
</Note>
...
Harness 服务的创建、配置和部署见 [Harness 服务部署](/productions/veadk/preview/zh/deploy/harness)。
...
设置 `enable_dynamic_load_skills=True` 后，智能体在每轮开始前重新从配置的技能来源（本地目录或云端技能空间）加载技能，检测新增、修改或删除的技能后自动更新技能列表、指令和工具集，无需重启。

<Note>
`enable_dynamic_load_skills` 可独立于 `skills` 使用：即使未配置初始技能列表，启用后仍会初始化技能工具集并注册刷新回调。
</Note>
