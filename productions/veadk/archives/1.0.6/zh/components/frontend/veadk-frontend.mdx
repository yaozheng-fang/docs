...
en_link: "/productions/veadk/archives/1.0.6/en/components/frontend/veadk-frontend"
---
...
- **对话与调试**：与智能体多轮对话，展示思考过程、工具调用、Token 与用时；对话中还会渲染智能体返回的 [A2UI](/productions/veadk/archives/1.0.6/zh/components/frontend/a2ui) 富界面卡片。
...
这些限制不会代替身份认证。公网部署仍应启用 SSO 或受信任的上游网关，并按最小权限原则配置云资源凭证。只需要创建和管理能力时，可以使用[Studio 智能体工作台](/productions/veadk/archives/1.0.6/zh/components/frontend/studio)。
...
一个自定义组件由两个部分组成，它们共享同一个 catalog id。后端部分见 [A2UI](/productions/veadk/archives/1.0.6/zh/components/frontend/a2ui#自定义组件)；前端部分如下。
