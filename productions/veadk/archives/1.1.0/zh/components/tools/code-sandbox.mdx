---
title: "代码沙箱"
en_link: "/productions/veadk/archives/1.1.0/en/components/tools/code-sandbox"
---
...
- `AGENTKIT_SKILL_INVOCATION_MODE`：`execute_skills` 调用模式，未指定时默认 `execute`
...
| 调用模式 | 说明 |
| :--- | :--- |
| `execute` | 默认模式。通过 Skill HTTP API 的 `/v1/skills/execute` 接口同步执行技能工作流。 |
| `run_sse` | 通过 ADK Runtime 的 `/run_sse` 接口以 SSE 方式执行，适配只暴露 ADK Runtime 接口的 Skill 沙箱。 |
| `a2a` | 通过 A2A 协议的 `message/send` 方法同步等待执行结果，适配使用 A2A 协议的 Skill 沙箱。 |
| `python_agent` | 旧版 RunCode 路径，在沙箱内执行 `agent.py`。传入 `env_vars` 时始终使用此模式。 |
| `skill_api` | `execute` 的别名，行为与 `execute` 一致。 |

<Note>
传入 `env_vars` 参数时，无论 `invocation_mode` 取值如何，`execute_skills` 始终使用 `python_agent` 模式，因为自定义环境变量注入依赖进程级环境变量。
</Note>
...
| 参数 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `invocation_mode` | `Optional[str]` | `None` | 技能沙箱调用模式。未指定时读取 `AGENTKIT_SKILL_INVOCATION_MODE` 环境变量，均未设置时使用 `execute`。 |
