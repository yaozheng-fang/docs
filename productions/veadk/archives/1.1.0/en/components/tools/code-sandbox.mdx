...
zh_link: "/productions/veadk/archives/1.1.0/zh/components/tools/code-sandbox"
---
...
- `AGENTKIT_SKILL_INVOCATION_MODE`: invocation mode for `execute_skills`; defaults to `execute`
...
## Skill sandbox invocation modes

`execute_skills` accepts an `invocation_mode` parameter to select the skill sandbox invocation backend, adapting to different Skill sandbox image versions. The mode can also be set globally with the `AGENTKIT_SKILL_INVOCATION_MODE` environment variable; the parameter takes precedence over the environment variable.

| Mode | Description |
| :--- | :--- |
| `execute` | Default mode. Runs the skill workflow synchronously through the Skill HTTP API `/v1/skills/execute` endpoint. |
| `run_sse` | Runs via the ADK Runtime `/run_sse` endpoint using SSE, for skill sandboxes that only expose the ADK Runtime interface. |
| `a2a` | Sends a `message/send` request via the A2A protocol and waits synchronously for the result, for skill sandboxes that use the A2A protocol. |
| `python_agent` | Legacy RunCode path that runs `agent.py` inside the sandbox. Always used when `env_vars` is provided. |
| `skill_api` | Alias for `execute`; behaves identically. |

<Note>
When `env_vars` is provided, `execute_skills` always uses `python_agent` mode regardless of `invocation_mode`, because custom environment variable injection relies on process-level environment variables.
</Note>
...
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `invocation_mode` | `Optional[str]` | `None` | Skill sandbox invocation mode. When not specified, the `AGENTKIT_SKILL_INVOCATION_MODE` environment variable is read; if both are unset, `execute` is used. |

### Example

```python title="skill_invocation_mode.py" lines
import asyncio
...
def run_skill_workflow(workflow_prompt: str, tool_context: ToolContext) -> str:
    """Run a workflow in the skills sandbox using A2A mode."""
    return execute_skills(
        workflow_prompt=workflow_prompt,
        tool_context=tool_context,
        invocation_mode="a2a",
