...
zh_link: "/productions/veadk/archives/1.1.3/zh/components/frontend/veadk-frontend"
---
...
| `--provider` | `volcengine` \| `byteplus` | Cloud provider for AgentKit services. Defaults to `volcengine`; selecting `byteplus` reads credentials from `BYTEPLUS_ACCESS_KEY`, `BYTEPLUS_SECRET_KEY`, and the optional `BYTEPLUS_SESSION_TOKEN`, with runtime listings defaulting to the region in `BYTEPLUS_REGION` or `ap-southeast-1`. |
