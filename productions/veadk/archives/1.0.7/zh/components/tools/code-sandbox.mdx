---
title: "代码沙箱"
en_link: "/productions/veadk/archives/1.0.7/en/components/tools/code-sandbox"
---
...
| `language` | `str` | — | 执行代码时使用 `python3`；执行 Shell 命令时使用 `bash` 或 `shell`。 |
...
`exec_dir`、`env`、`hard_timeout` 和 `max_output_length` 仅在 `language` 为 `bash` 或 `shell` 时生效。
