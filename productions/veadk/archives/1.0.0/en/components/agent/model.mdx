---
title: "Model"
zh_link: "/productions/veadk/archives/1.0.0/zh/components/agent/model"
---

By default an agent uses the globally configured model — the one set via
environment variables or `config.yaml` (see the [Quickstart](/productions/veadk/archives/1.0.0/en/get-started/quickstart)).
...
<Note>
  In VeADK 1.0.0, model fallback applies only to the default model API. When the Responses API is enabled, pass a single string to `model_name`.
</Note>
