---
title: "运行时"
en_link: "/productions/veadk/archives/1.0.0/en/components/agent/runtime"
---
...
<Warning>
  Codex 运行时可能根据 Codex SDK 的策略执行命令和访问文件。请在隔离环境中运行可信任务，并限制进程可访问的凭证和数据。VeADK 1.0.0 不会覆盖 SDK 的沙箱、网络和操作审核默认值。
</Warning>
