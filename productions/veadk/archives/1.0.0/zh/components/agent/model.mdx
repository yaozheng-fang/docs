---
title: "模型"
en_link: "/productions/veadk/archives/1.0.0/en/components/agent/model"
---

智能体默认使用全局配置的模型，即通过环境变量或 `config.yaml` 设置的模型（参见[快速开始](/productions/veadk/archives/1.0.0/zh/get-started/quickstart)）。也可以在创建智能体时为其单独指定模型。

在火山引擎环境中，1.0.0 的默认模型为 `doubao-seed-1-8-251228`，provider 为 `openai`，接口地址为 `https://ark.cn-beijing.volces.com/api/v3/`。当 `CLOUD_PROVIDER=byteplus` 时，默认模型为 `seed-1-6-250915`，接口地址为 `https://ark.ap-southeast.bytepluses.com/api/v3`。
...
<Note>
  VeADK 1.0.0 的回退模型仅适用于默认模型接口。启用 Responses API 时，`model_name` 必须传入单个字符串。
</Note>
