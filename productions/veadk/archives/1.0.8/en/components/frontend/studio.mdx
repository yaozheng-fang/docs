...
- You have the ID of an AgentKit agent center that contains an invokable remote agent, and the current credentials can access that center.
...
<Steps>
  ...
  <Step title="Configure the agent center">
    Enter the AgentKit agent-center ID. If the center is not in Beijing, set its region under More options.
  </Step>
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| AgentKit agent center ID | `REGISTRY_SPACE_ID` | `str` | Yes | — | ID of the center used to discover remote agents. |
...
| AgentKit agent center region | `REGISTRY_REGION` | `str` | No | `cn-beijing` | Region that contains the agent center. |
...
| Project generation or testing cannot start | Confirm that the remote agent is below the root agent and that the agent-center ID is present. |
| The agent center cannot be accessed | Confirm that the center ID and region are correct and that the current credentials can access the center. |
...
| `--veadk-version` | `str` | Latest release | `veadk-python` version installed in VeFaaS. Set it to `1.0.8` to reproduce this version. |
