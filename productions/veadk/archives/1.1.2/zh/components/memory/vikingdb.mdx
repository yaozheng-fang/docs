...
en_link: "/productions/veadk/archives/1.1.2/en/components/memory/vikingdb"
---
...
- `region`：`byteplus` 模式下固定为 `cn-hongkong`，忽略环境变量与显式传入的值；其余模式未显式设置时读取环境变量 `DATABASE_VIKING_REGION`，仍为空时默认为 `cn-beijing`。
...
| `DATABASE_VIKING_REGION` | `cn-beijing`（`byteplus` 模式忽略此变量，固定为 `cn-hongkong`） | 记忆库区域。 |
