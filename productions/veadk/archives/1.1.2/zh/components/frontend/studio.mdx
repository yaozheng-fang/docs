---
title: "Studio 智能体工作台"
en_link: "/productions/veadk/archives/1.1.2/en/components/frontend/studio"
---

AgentKit Studio 使用与 VeADK Frontend 相同的服务和完整界面，提供对话、搜索、历史会话、库、视频创作、自动化集成，以及智能体创建、测试、部署和管理功能，启动后默认进入对话页面。智能体工作台以画布形式呈现多智能体拓扑，并集中展示已部署 Runtime 的版本与部署状态，支持在同一 Runtime 上迭代更新。

当前可通过自定义配置、代码包部署或从存量项目迁移创建项目。你可以预览和编辑生成的文件，启动临时测试进程，将项目下载为 ZIP，或者部署到 AgentKit。Studio 还支持选择云端 Runtime、多来源技能、多模态会话、自动化集成，以及集中查看和重试部署任务。
...
1. 在「添加智能体」中选择「从 0 快速创建」进入自定义配置，选择「从代码包添加和部署」上传已有项目，或选择「从存量项目迁移」将 LangChain、Dify 等框架的项目迁移至 VeADK。
...
创建新 Runtime 时，访问鉴权默认为 API Key；如需改用用户身份验证，可在部署配置区选择 VeIdentity 用户池。Studio 服务端使用自身配置的火山引擎凭证加载当前账号可见的用户池列表，浏览器不接触凭证。列表中会标记当前 Studio 登录所用的用户池：选择该用户池时，Studio 会把登录后验证的 JWT 转发给 Runtime，调用方无需另行获取令牌；选择其他用户池时，调用方需使用该用户池签发的 JWT 访问 Runtime。用户池所在地域通过 `VEIDENTITY_REGION` 环境变量确定，默认为当前云服务商的默认地域（火山引擎为 `cn-beijing`，BytePlus 为 `ap-southeast-1`）。
...
<Note>
  当内置智能体的会话结束后留下了可恢复的快照时，管理员会在沙箱会话列表中看到状态为「可唤醒」的智能体快照。点击「唤醒」可从快照恢复一个就绪的会话并继续对话；也可以删除不再需要的快照。快照详情页展示快照状态、原因、Snapshot ID 和来源 Session ID。该能力仅对 `admin` 角色开放，且仅在已配置对应的沙箱快照 Tool 时可用。
</Note>
...
侧边栏的「库」页面集中管理技能、知识库和产物，分为三个标签页：
...
| 网页 | 公开网页 URL | Studio 服务端抓取网页并提取正文内容为 Markdown 后存入知识库。 |
...
更新过程中，部署进度面板实时展示 VeFaaS 部署日志。日志在服务端经过过滤，去除 curl 进度条、配置 JSON 导出、ANSI 转义序列和重复行，仅保留与部署相关的有效内容。当部署所用凭证缺少 VeFaaS 日志查询权限时，日志面板自动隐藏，不影响更新继续进行。更新完成后，Studio 会自动刷新页面以加载新版本；刷新前如有未关闭的更新对话框，也会在重新打开页面后自动恢复显示。
...
登录后，侧边栏底部的账号菜单提供「系统信息」入口，选择后进入系统信息页面，展示 Studio 的版本、存储、沙箱信息和用户池。页面中的 TOS 存储桶、沙箱 Tool 和用户池均提供跳转到对应云服务商控制台的链接，点击后在新标签页打开。该页面仅对 `admin` 角色开放，所有资源标识均为只读。
...
列出 Studio 已配置的沙箱 Tool 及其 ID，包括 Codex Sandbox（`SANDBOX_CHAT_CODEX`）、OpenClaw Sandbox（`SANDBOX_CHAT_OPENCLAW`）、Hermes Sandbox（`SANDBOX_CHAT_HERMES`）和 Dev Sandbox（`SANDBOX_DEV`）。已配置的 Tool ID 提供跳转到云控制台对应 Tool 详情页的链接；未配置的 Tool 显示「未配置」。
