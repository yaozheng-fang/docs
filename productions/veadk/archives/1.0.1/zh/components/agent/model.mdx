---
title: "模型"
en_link: "/productions/veadk/archives/1.0.1/en/components/agent/model"
---

智能体默认使用全局配置的模型，即通过环境变量或 `config.yaml` 设置的模型（参见[快速开始](/productions/veadk/archives/1.0.1/zh/get-started/quickstart)）。也可以在创建智能体时为其单独指定模型。
...
<Note>
  VeADK 1.0.1 的回退模型仅适用于默认模型接口。启用 Responses API 时，`model_name` 必须传入单个字符串。
</Note>
