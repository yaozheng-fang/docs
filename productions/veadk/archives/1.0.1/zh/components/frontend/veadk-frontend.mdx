...
- **对话与调试**：与智能体多轮对话，展示思考过程、工具调用、Token 与用时；对话中还会渲染智能体返回的 [A2UI](/productions/veadk/archives/1.0.1/zh/components/frontend/a2ui) 富界面卡片。
...
| `--auth-mode` | `frontend` | 认证模式，可取 `frontend` 或 `gateway`；也可通过 `VEADK_FRONTEND_AUTH_MODE` 设置。 |
...
**网关认证** —— 当服务部署在已完成用户认证的 AgentKit Runtime 网关之后时，可使用 `gateway` 模式：

```bash
veadk frontend --agents-dir examples --auth-mode gateway
```

该模式不运行前端内置的 OAuth2 登录，而是从上游转发的 `Authorization: Bearer <JWT>` 中读取用户标识。

<Warning>
  `gateway` 模式信任上游已经验证 JWT，不会再次校验签名。仅应在请求必须经过可信认证网关、且客户端无法绕过网关直接访问 VeADK Frontend 的环境中使用。
</Warning>
...
一个自定义组件由两个部分组成，它们共享同一个 catalog id。后端部分见 [A2UI](/productions/veadk/archives/1.0.1/zh/components/frontend/a2ui#自定义组件)；前端部分如下。
