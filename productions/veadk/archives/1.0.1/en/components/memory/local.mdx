...
For a full cross-session example, see [Long-term memory · Cross-session example](/productions/veadk/archives/1.0.1/en/components/memory/index#cross-session-example).
...
<Warning>
The `local` backend keeps memory only in process memory; data is lost when the process exits and cannot be shared across processes. For persistence or multi-instance sharing, use [VikingDB](/productions/veadk/archives/1.0.1/en/components/memory/vikingdb), [Mem0](/productions/veadk/archives/1.0.1/en/components/memory/mem0), [OpenSearch](/productions/veadk/archives/1.0.1/en/components/memory/opensearch), or [Redis](/productions/veadk/archives/1.0.1/en/components/memory/redis).
</Warning>
