...
zh_link: "/productions/veadk/archives/1.0.10/zh/components/knowledge/context-search"
---
...
<Warning>
`index` (the scene ID) must be a numeric string, otherwise initialization fails; initialization also calls `GetScene` to verify the scene exists. Before retrieval, `context_search_engine_endpoint` and `context_search_engine_apikey` must be set, otherwise `search` fails.
</Warning>
