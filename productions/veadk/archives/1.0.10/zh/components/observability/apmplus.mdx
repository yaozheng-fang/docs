---
title: "APMPlus"
en_link: "/productions/veadk/archives/1.0.10/en/components/observability/apmplus"
---
...
<Note>
`app_key` 未通过环境变量提供时，导出器会调用火山引擎鉴权自动获取 APMPlus 令牌，因此需要配置有效的火山引擎凭证。`endpoint` 使用 gRPC 协议、以非加密方式连接。
</Note>
...
<Note>
如果初始化 `OpentelemetryTracer` 时已存在全局 `TracerProvider`，VeADK 将复用该 provider 并自动移除 `APMPlusExporter`，因为已存在的 provider 被视为已负责 APMPlus 的链路数据上报。可通过 `apmplus_managed_externally` 属性确认此状态，详见[可观测概述](/productions/veadk/preview/zh/components/observability)。
</Note>
