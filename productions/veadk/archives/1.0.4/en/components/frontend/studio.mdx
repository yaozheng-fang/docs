...
zh_link: "/productions/veadk/archives/1.0.4/zh/components/frontend/studio"
---

VeADK Studio is a workbench for creating and managing agents. It uses the same service as VeADK Frontend, but hides chat, search, session history, and the skill center and opens directly on the Add Agent page.

Studio can generate a project through an intelligent flow, custom configuration, or a template. You can preview and edit generated files, download the project as a ZIP, or deploy it to AgentKit. The workflow entry is marked as coming soon and cannot be selected. Deployment progress is presented in build-image, deploy, and publish stages.
...
1. On Add Agent, select the intelligent, custom, or template path. The workflow entry is not available.
2. Configure the model, instruction, tools, skills, memory, and knowledge base. Multi-agent projects can also use sequential, parallel, loop, or A2A nodes.
3. Review the generated files and download a ZIP if you need to work offline.
4. Select AgentKit deployment and monitor the build-image, deploy, and publish stages.

The online Test Run action for generated projects was removed in 1.0.4. To validate code before deployment, download it and run tests in a controlled local development environment.
...
| `--dev` | Boolean flag | `false` | Serve only the API and allow CORS from the Vite development server at `http://localhost:5173`. |
...
| `--open` / `--no-open` | `bool` | `--no-open` | Open the default browser when ready. Ignored in development mode. |
...
| `--region` | `str` | `cn-beijing` | Region for VeFaaS, API Gateway, and related resources. |
...
| `--veadk-version` | `str` | Latest release | `veadk-python` version installed in VeFaaS. Set it to `1.0.4` to reproduce this version. |
