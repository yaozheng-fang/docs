...
- **管理智能体**：按当前用户列出通过工作台部署的 AgentKit Runtime，查看控制面配置、资源和主智能体信息，或删除不再使用的 Runtime。
- **对话与调试**：与智能体多轮对话，展示思考过程、工具调用、Token 与用时；对话中还会渲染智能体返回的 [A2UI](/productions/veadk/archives/1.0.4/zh/components/frontend/a2ui) 富界面卡片。
- **智能体拓扑**：在对话中展示顺序、并行、循环、LLM 与 A2A 节点组成的智能体结构和转移路径。
...
| `--auth-mode` | `frontend` | 身份来源。`frontend` 由应用处理 OAuth2 登录；`gateway` 信任上游网关转发的 JWT 身份。环境变量 `VEADK_FRONTEND_AUTH_MODE`。 |
| `--open` / `--no-open` | `--no-open` | 服务就绪后是否打开默认浏览器；开发模式下忽略。 |
...
VeADK Frontend 可能部署到公网。1.0.4 对用户可控制的地址和文件路径施加以下限制：

- 在线创建项目不再直接执行浏览器提交的代码；部署前测试应在受控的本地环境中完成。
...
这些限制不会代替身份认证。公网部署仍应启用 SSO 或受信任的上游网关，并按最小权限原则配置云资源凭证。只需要创建和管理能力时，可以使用[Studio 智能体工作台](/productions/veadk/archives/1.0.4/zh/components/frontend/studio)。
...
一个自定义组件由两个部分组成，它们共享同一个 catalog id。后端部分见 [A2UI](/productions/veadk/archives/1.0.4/zh/components/frontend/a2ui#自定义组件)；前端部分如下。
