...
<Note>
向量类后端（`local`、`opensearch`、`redis`）会对记忆做向量化，需要安装扩展依赖：`pip install "veadk-python[extensions]"`，并配置 embedding 模型（环境变量前缀 `MODEL_EMBEDDING_`，缺省时复用 `MODEL_AGENT_API_KEY`）。`viking`、`mem0` 与 `openviking` 是服务端后端，无需本地 embedding。
</Note>
...
调试可以用 `local`，生产环境可以根据数据所在地和服务能力选择 `viking`、`mem0` 或 `openviking`。

| 后端 | 存储 | 依赖 | 适用场景 | 文档 |
| :--- | :--- | :--- | :--- | :--- |
| `local` | 内存向量索引 | `extensions` + embedding | 本地调试 | [本地内存](/productions/veadk/archives/1.0.4/zh/components/memory/local) |
| `viking` | VikingDB 记忆库（托管） | 火山引擎账号 | 生产推荐 | [VikingDB](/productions/veadk/archives/1.0.4/zh/components/memory/vikingdb) |
| `mem0` | Mem0 记忆库（托管） | Mem0 API Key | 生产推荐 | [Mem0](/productions/veadk/archives/1.0.4/zh/components/memory/mem0) |
| `openviking` | OpenViking 长期记忆 | OpenViking 服务与 API Key | 从完整会话提取用户实体、事件和偏好 | [OpenViking](/productions/veadk/archives/1.0.4/zh/components/memory/openviking) |
| `opensearch` | OpenSearch 向量库 | OpenSearch + embedding | 自建向量检索 | [OpenSearch](/productions/veadk/archives/1.0.4/zh/components/memory/opensearch) |
| `redis` | Redis 向量库 | Redis(RediSearch) + embedding | 自建向量检索 | [Redis](/productions/veadk/archives/1.0.4/zh/components/memory/redis) |
