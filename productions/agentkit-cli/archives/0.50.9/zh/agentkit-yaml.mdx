---
title: "agentkit.yaml"
en_link: "/productions/agentkit-cli/archives/0.50.9/en/agentkit-yaml"
---
...
| `cloud_provider` | 云厂商：`volcengine` 或 `byteplus`。用于 Runtime、IAM/STS、CR、TOS 与 Code Pipeline。 | 当前云环境 |
| `region` | 各资源默认继承的部署区域。火山引擎可用 `cn-beijing` 或 `cn-shanghai`；BytePlus 使用 `ap-southeast-1`。 | — |
| `project` | 所属 AgentKit 项目。 | `default` |

`cloud_provider` 是一次部署的统一云厂商选择。资源块只能覆盖 `region` 与 `project`，不支持在一次部署中混用不同云厂商。命令行显式指定的 `--provider` 优先于环境探测，并写入新生成的配置。
...
供值方式见[构建与部署 · 环境变量](/productions/agentkit-cli/archives/0.50.9/zh/commands/deploy#环境变量)：本地部署可导出变量或写入本机 `.env`，持续部署则配成仓库 Secret。用于部署鉴权的 `VOLCENGINE_*` 不会注入运行时。
...
| `frontend.gateway` | 指定已有的 serverless 网关。省略时跨项目查找可用网关；没有可用网关时自动创建。 |
...
`dockerfile` 字段指定构建镜像所用的 Dockerfile 路径，默认为 `.agentkit/Dockerfile`；若项目根目录存在 `./Dockerfile` 则改用它。容器如何启动、以及必须监听 `0.0.0.0:8000` 的约定，见[构建与部署 · 容器入口与端口](/productions/agentkit-cli/archives/0.50.9/zh/commands/deploy#容器入口与端口)。
