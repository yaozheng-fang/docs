---
title: "agentkit.yaml"
zh_link: "/productions/agentkit-cli/archives/0.50.9/zh/agentkit-yaml"
---
...
```yaml .agentkit/agentkit.yaml lines
...
| `region` | Default deploy region inherited by resources. Volcengine supports `cn-beijing` and `cn-shanghai`; BytePlus uses `ap-southeast-1`. | — |
| `project` | The AgentKit project it belongs to. | `default` |

`cloud_provider` selects one provider for the entire deployment. Resource blocks can override only `region` and `project`; mixing providers in one deployment is unsupported. An explicit command-line `--provider` takes precedence over environment detection and is written to newly generated configuration.
...
The `dockerfile` field sets the path to the Dockerfile used for the build, defaulting to `.agentkit/Dockerfile` (or `./Dockerfile` if one exists at the project root). For how the container starts and the requirement to listen on `0.0.0.0:8000`, see [Build & deploy · Container entrypoint and port](/productions/agentkit-cli/archives/0.50.9/en/commands/deploy#container-entrypoint-and-port).
