---
title: "迁移已有智能体"
en_link: "/productions/agentkit-cli/archives/0.51.2/en/commands/migrate"
---
...
使用 `--framework dify create` 时，输入目录应包含 `workflow.yml` 或 `workflow.yaml`。CLI 会在远程迁移开始前读取 Dify DSL，向迁移过程提供只读上下文：DSL 版本、应用模式、节点与边、变量、App Features、依赖、诊断信息以及被截断内容的位置。凭证字段和常见认证文本会被脱敏。
