...
zh_link: "/productions/agentkit-cli/archives/0.50.1/zh/workflows/feishu"
---
...
<Note>
  First: complete [authentication](/productions/agentkit-cli/archives/0.50.1/en/commands/auth) (deploy uses AK/SK). Then create an app on the [Feishu Open Platform](https://open.feishu.cn/), enable the **bot** capability, and note the **App ID** and **App Secret** (Feishu has no API to create an app — do this once in the console).
</Note>
...
- **Runtime auth**: on the Feishu path the runtime uses `key_auth` (the proxy holds the API key). For web login with per-user identity forwarding, see [Deploy a frontend with SSO login](/productions/agentkit-cli/archives/0.50.1/en/workflows/frontend-sso).
