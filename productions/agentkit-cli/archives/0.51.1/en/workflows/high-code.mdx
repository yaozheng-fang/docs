...
zh_link: "/productions/agentkit-cli/archives/0.51.1/zh/workflows/high-code"
---
...
<Note>
  Configure AK/SK credentials first, or obtain short-lived STS credentials through [SSO login](/productions/agentkit-cli/archives/0.51.1/en/commands/auth). `launch`, `deploy`, and `invoke` use the same credential precedence to resolve valid credentials. See the Authentication section of the [Quickstart](/productions/agentkit-cli/archives/0.51.1/en/quickstart). For the full guide to writing agent code, see the [VeADK 1.0.9 docs](/productions/veadk/archives/1.0.9/en/index).
</Note>
...
<Steps>
  ...
  <Step title="Build and deploy">
    ...
    <Tip>
      To inspect or hand-edit `agentkit.yaml` or the `Dockerfile` between scaffolding and building, run `agentkit config --show`, then split the flow into `agentkit build` and `agentkit deploy`. See [Config](/productions/agentkit-cli/archives/0.51.1/en/commands/config), [Build](/productions/agentkit-cli/archives/0.51.1/en/commands/build), and [Deploy](/productions/agentkit-cli/archives/0.51.1/en/commands/deploy).
    </Tip>
  </Step>
...
When you only need configuration and no code, use [Using Harness](/productions/agentkit-cli/archives/0.51.1/en/workflows/harness) instead.
