---
title: "接入飞书机器人"
en_link: "/productions/agentkit-cli/archives/0.51.1/en/workflows/feishu"
---
...
<Note>
  开始前：按照[鉴权与登录](/productions/agentkit-cli/archives/0.51.1/zh/commands/auth)配置 AK/SK 或完成 SSO 登录。并在[飞书开放平台](https://open.feishu.cn/)创建一个应用，开启**机器人**能力，记下 **App ID** 与 **App Secret**（飞书没有创建应用的开放接口，这一步在控制台手动完成）。
</Note>
...
- **运行时鉴权**：飞书链路下运行时用 `key_auth`，由 proxy 持有 API key。若要网页登录并透传用户身份，见[部署带 SSO 登录的前端](/productions/agentkit-cli/archives/0.51.1/zh/workflows/frontend-sso)。
