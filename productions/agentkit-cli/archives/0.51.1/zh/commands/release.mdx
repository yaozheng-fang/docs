---
title: "发布"
en_link: "/productions/agentkit-cli/archives/0.51.1/en/commands/release"
---

`release` 使用 `.agentkit/agentkit.yaml` 执行完整发布流程：生成发布配置、云端构建镜像、创建或更新 AgentKit Runtime，并按配置发布飞书、企业微信、钉钉代理或前端 BFF。若只使用根目录 `agentkit.yaml` 的生命周期配置，请使用 [`launch`](/productions/agentkit-cli/archives/0.51.1/zh/commands/launch)。
...
消息渠道与前端发布配置仍写在 `.agentkit/agentkit.yaml`，字段说明见 [agentkit.yaml](/productions/agentkit-cli/archives/0.51.1/zh/agentkit-yaml)。
