---
title: "部署带 SSO 登录的前端"
en_link: "/productions/agentkit-cli/archives/0.50.4/en/workflows/frontend-sso"
---
...
<Note>
  开始前：完成[鉴权与登录](/productions/agentkit-cli/archives/0.50.4/zh/commands/auth)（部署走 AK/SK 控制面）；并在 [Agent Identity](https://console.volcengine.com/identity) 准备一个**用户池**与一个 **WEB 客户端**，记下 `user_pool_id` 与 `client_id`（客户端 secret 由 CLI 自动获取，无需手动填）。要用飞书登录，则在用户池里把飞书配成身份来源（第三方身份联合）。
</Note>
...
只需要机器人渠道、不需要网页登录时，改用[接入飞书机器人](/productions/agentkit-cli/archives/0.50.4/zh/workflows/feishu)。
