---
title: "agentkit.yaml"
en_link: "/productions/agentkit-cli/archives/0.50.6/en/agentkit-yaml"
---
...
供值方式见[构建与部署 · 环境变量](/productions/agentkit-cli/archives/0.50.6/zh/commands/deploy#环境变量)：本地部署可导出变量或写入本机 `.env`，持续部署则配成仓库 Secret。用于部署鉴权的 `VOLCENGINE_*` 不会注入运行时。
...
| `frontend.gateway` | 指定已有的 serverless 网关。省略时复用当前项目中的网关；没有可用网关时自动创建。 |
...
`dockerfile` 字段指定构建镜像所用的 Dockerfile 路径，默认为 `.agentkit/Dockerfile`；若项目根目录存在 `./Dockerfile` 则改用它。容器如何启动、以及必须监听 `0.0.0.0:8000` 的约定，见[构建与部署 · 容器入口与端口](/productions/agentkit-cli/archives/0.50.6/zh/commands/deploy#容器入口与端口)。
