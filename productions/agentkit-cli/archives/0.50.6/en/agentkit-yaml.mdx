---
title: "agentkit.yaml"
zh_link: "/productions/agentkit-cli/archives/0.50.6/zh/agentkit-yaml"
---
...
| `frontend.gateway` | Select an existing serverless gateway. When omitted, reuse one in the current project or create one if none is available. |
...
The `dockerfile` field sets the path to the Dockerfile used for the build, defaulting to `.agentkit/Dockerfile` (or `./Dockerfile` if one exists at the project root). For how the container starts and the requirement to listen on `0.0.0.0:8000`, see [Build & deploy · Container entrypoint and port](/productions/agentkit-cli/archives/0.50.6/en/commands/deploy#container-entrypoint-and-port).
