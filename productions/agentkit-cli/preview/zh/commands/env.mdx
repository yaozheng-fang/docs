---
title: "管理环境"
en_link: "/productions/agentkit-cli/preview/en/commands/env"
---
...
```yaml title=".agentkit/environment.yaml" lines
version: 1
type: self-host
status: ready
tool:
...
`--force-new` 创建额外的运行时，不会自动销毁先前的运行时。仅在明确需要另一个环境时使用，并保存旧 Runtime ID 以便后续管理；重试失败操作前先运行 `env status`，确认是否已有部分资源创建成功

包含真实 Anthropic 凭证的 `.agentkit/sandbox.yaml` 和环境状态文件应保留在本机，不提交到版本库。`env status` 隐藏环境变量输出并不等于本地文件不含凭证
