---
title: "运行评测实验"
en_link: "/productions/agentkit-cli/preview/en/commands/eval/run"
---

`eval run` 是串联评测闭环的核心命令：它以一个评测集、一个或多个评估器，对一个已部署的运行时或 TEA 源评测目标提交一次实验。评测集、评估器与目标均可用 ID 或名称指定，字段映射默认自动完成。

先准备当前后端中的评测集、评估器和可用目标；TEA 还需先提交评估器版本。下方先预览请求，`--dry-run` 仍需要联网解析资源，但不会提交实验，也不会自动发布评测集草稿

<Warning>
移除 `--dry-run` 后会调用目标与评判模型，可能产生用量费用并把用例、输出交给评测平台。未指定已提交评测集版本时，还可能自动发布草稿版本
</Warning>
