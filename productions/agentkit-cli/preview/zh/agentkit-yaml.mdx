---
title: "agentkit.yaml"
en_link: "/productions/agentkit-cli/preview/en/agentkit-yaml"
---
...
<Warning>
根目录生命周期配置不会把 `${VAR}` 替换为环境变量。需要传入模型密钥时，可在已导出变量后运行 `agentkit config --runtime_envs "MODEL_AGENT_API_KEY=$MODEL_AGENT_API_KEY"`，由 shell 先展开值；这会将实际密钥写入 `agentkit.yaml`，请先将该文件与 `.env` 加入 `.gitignore` 和 `.dockerignore`。下文 `.agentkit/agentkit.yaml` 的 `${VAR}` 语法只适用于 Release 配置
</Warning>
...
| `runtime_gateway_mode` | 运行时网关模式 `Shared` 或 `Exclusive` | `Shared` |
| `runtime_gateway_instance_id` | 专属网关实例 ID，Exclusive 模式必填 | — |
...
<Warning>
当前托管 Harness Sidecar 仅支持 火山引擎 `cn-shanghai` 与 `cn-beijing`、CPython 3.12、`linux/amd64`，并要求 `runtime.min_instance` 与 `runtime.max_instance` 都为 `1`。它还要求发布环境由平台提供托管 Sidecar 基础镜像配置；普通本地环境未提供该配置时，发布会失败。启用 Sidecar 时必须使用 Header API Key 形式的 `key_auth` 网关鉴权，不能与 `custom_jwt` 或前端 BFF 鉴权派生同时使用。
</Warning>
...
| 字段 | 说明 |
| --- | --- |
| `role_name` | IAM 角色名称；创建时校验显式角色，未指定时优先复用符合默认权限要求的角色，否则创建新角色；更新保留原角色 |
...
生命周期配置在 `launch_types.cloud` 或 `launch_types.hybrid` 中使用 `runtime_gateway_mode` 和 `runtime_gateway_instance_id`；发布配置在 `runtime` 中使用 `gateway_mode` 和 `gateway_instance_id`
...
| 标志/参数 | 说明 | 默认值 |
| --- | --- | --- |
| `runtime.gateway_mode` | 网关模式，`Shared` 或 `Exclusive` | `Shared` |
| `runtime.gateway_instance_id` | 已有专属网关实例 ID | Exclusive 模式必填 |

Exclusive 模式不能同时设置运行时网络配置，网络跟随网关实例；Shared 模式不能指定网关实例 ID。网关绑定在创建后不可更改，更新配置与原绑定冲突时会停止发布
...
启用前端 BFF 与 Identity 登录时，前端代理保留登录回调、会话 Cookie 与返回路径，使登录后请求能够继续使用原会话。保持前端配置中的 issuer、client ID 与已登记回调地址一致，无需为此修正增加新的配置字段
