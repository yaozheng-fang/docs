...
没有 AK/SK 时，可通过控制台获取临时凭据；已有 SSO profile 时，`--console` 可明确选择控制台方式
...
远程服务器使用 `agentkit login --remote`。凭据优先级与自动登录规则见 [鉴权与登录](/productions/agentkit-cli/preview/zh/commands/auth#控制台登录与凭据优先级)
