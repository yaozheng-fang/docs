...
## Sign in through the console

Use console login to obtain temporary credentials without configuring access keys. `--console` explicitly selects console login when an SSO profile already exists
...
On a remote server, use `agentkit login --remote`. See [authentication](/productions/agentkit-cli/preview/en/commands/auth#console-login-and-credential-precedence) for credential precedence and automatic login
