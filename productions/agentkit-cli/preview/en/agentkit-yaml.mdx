---
title: "agentkit.yaml"
zh_link: "/productions/agentkit-cli/preview/zh/agentkit-yaml"
---
...
```yaml title="agentkit.yaml" lines
common:
  agent_name: my-agent
...
<Warning>
Root lifecycle configuration does not substitute `${VAR}` from the environment. After exporting a model key, use `agentkit config --runtime_envs "MODEL_AGENT_API_KEY=$MODEL_AGENT_API_KEY"` so the shell expands it first. This writes the actual key to `agentkit.yaml`; add that file and `.env` to both `.gitignore` and `.dockerignore` before running the command. The `${VAR}` syntax described below applies to Release configuration in `.agentkit/agentkit.yaml`
</Warning>
...
| `runtime_gateway_mode` | Runtime gateway mode: `Shared` or `Exclusive` | `Shared` |
| `runtime_gateway_instance_id` | Exclusive gateway instance ID, required in Exclusive mode | — |
...
# Volcengine cn-shanghai/cn-beijing, CPython 3.12, linux/amd64, and exactly one instance;
...
# ── Advanced (optional) ──────────────────────────────────
# role_name: ""                # IAM role; reuse a qualified role or create one when omitted
...
| Field | Description |
| --- | --- |
| `role_name` | IAM role name; explicitly configured roles are validated. Otherwise creation reuses a qualified role or creates one; updates retain the current role. |
...
Lifecycle configuration uses `runtime_gateway_mode` and `runtime_gateway_instance_id` under `launch_types.cloud` or `launch_types.hybrid`. Release configuration uses `gateway_mode` and `gateway_instance_id` under `runtime`

```yaml title=".agentkit/agentkit.yaml" lines
runtime:
  gateway_mode: Exclusive
  gateway_instance_id: gateway-example
```

| Flag / argument | Description | Default |
| --- | --- | --- |
| `runtime.gateway_mode` | Gateway mode: `Shared` or `Exclusive` | `Shared` |
| `runtime.gateway_instance_id` | Existing exclusive gateway instance ID | Required in Exclusive mode |
...
```bash lines
agentkit config --runtime_gateway_mode Exclusive --runtime_gateway_instance_id gateway-example
```

## Frontend login sessions

With frontend BFF and Identity authentication enabled, the frontend proxy preserves login callbacks, session cookies, and return paths so requests continue in the signed-in session. Keep the issuer, client ID, and registered callback URL consistent. This fix requires no additional configuration fields
