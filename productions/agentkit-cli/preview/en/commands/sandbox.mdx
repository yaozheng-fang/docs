---
title: "Use sandboxes"
zh_link: "/productions/agentkit-cli/preview/zh/commands/sandbox"
---
...
| `sandbox snapshot` | Create, query, resume, and delete session snapshots |
...
The command waits for the sandbox browser to become ready before opening its home page
...
| Flag / argument | Description | Default |
| --- | --- | --- |
| `--tool-id <id>` | sandbox tool ID. Resolve tool by CLI tool-id/tool-name, then sandbox.yaml config, env AGENTKIT_SANDBOX_TOOL_ID. | Required |
...
| `--sid <id>` | sandbox user session ID to snapshot | — |
...
| `--sid <id>` | sandbox user session ID filter | — |
| `--session <id>` | alias for --session-id | — |
| `-h, --help` | Show help | — |
...
| `--sid <id>` | user session ID to request for the resumed session | — |
| `--session <id>` | alias for --session-id | — |
| `-h, --help` | Show help | — |
