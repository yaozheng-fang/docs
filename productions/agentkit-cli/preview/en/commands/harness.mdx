---
title: "Harness"
zh_link: "/productions/agentkit-cli/preview/zh/commands/harness"
---
...
| `--mcp-server <json>` | Remote MCP server JSON; repeatable, or pass `[]` alone to clear the list | No override |
...
| `--short-term-memory-schema <value>` | PostgreSQL schema name | None |
...
Prepare Python 3 and the Harness server dependencies first. Startup output identifies the dependency file; if modules are missing, install that file with the same `--python` interpreter and retry. This command does not create a Python environment or install dependencies. Restart after editing `harness.yaml`; `--reload` watches server code
...
| `--cronjob` | Enable persistent scheduled tasks | `false` |
| `--cronjob-tos-bucket <bucket>` | Existing TOS bucket for task definitions, state, and results | `cronjob.tos_bucket` |
| `--cronjob-tos-region <region>` | TOS bucket region | Runtime region |
...
| `[message]` | User message; when omitted, read `message` or `prompt` from the invocation config | — |
...
| `--mcp-server <json>` | Remote MCP server JSON; repeatable, or pass `[]` alone to clear the list | No override |
| `-c, --config <path>` | Read YAML or JSON configuration for this invocation | — |
| `--harness-merge` | Ask the target runtime to merge overrides with its default Harness configuration | Omitted; server behavior applies |
| `--harness-enhance <json>` | Send a one-time `harness_enhance` JSON object; requires support in the target runtime | — |
...
| Flag / argument | Description | Default |
| --- | --- | --- |
| `mcp_servers` | Server list; an empty list disables remote MCP | `[]` |
| `protocol` | `streamable-http` or `sse` | `streamable-http` |
| `endpoint` | Required HTTP(S) URL without user information or a fragment | — |
| `api_key` | Optional credential sent as `Authorization: Bearer`; supports environment placeholders | — |
...
| Flag / argument | Description | Default |
| --- | --- | --- |
| `message` / `prompt` | Message string | Required in the file or positional argument |
| `region`, `rev` | Target region and runtime version | Resolved region; current version |
| `user_id`, `session_id` | User and session IDs | `agentkit_user`; random session |
| `token`, `tip_token_key`, `apikey` / `api_key` | Explicit invocation credentials | — |
| `raw` | Output raw responses | `false` |
| `protocol` | `invoke` or `run_sse` | `run_sse` |
| `max_llm_calls` | Positive maximum model call count for the run | Target runtime default |
| `harness` / `overrides` | Per-request agent override object | — |
| `harness.model_name`, `harness.system_prompt`, `harness.runtime` | Model, instruction, and runtime backend | No override |
| `harness.tools`, `harness.skills` | Comma-separated strings or lists of strings | No override |
| `harness.mcp_servers` | Remote MCP server list for the request | No override |
| `registry`, `registry_space_id`, `registry_space_name`, `registry_top_k`, `registry_endpoint`, `registry_region` | A2A registry and retrieval settings; `registry_top_k` is a positive integer | No override |
| `harness_merge` | Boolean sent to target runtimes that support merging defaults | Omitted |
| `harness_enhance` | Enhancement object; `components` accepts a string list or comma-separated string. Fields require target runtime support | Omitted |
...
With `cronjob` enabled, use `harness cronjob` to create recurring or one-time invocations, inspect results, and pause, resume, or cancel work. See [Harness scheduled tasks](/productions/agentkit-cli/preview/en/commands/harness-cronjob) for configuration, options, and limitations

## Verification and session storage

After deployment, run `agentkit runtime show my-harness` to check the active version, then invoke it with `agentkit harness invoke my-harness "hello"`. Runtime readiness alone does not verify model credentials, MCP services, or databases. Inspect `agentkit runtime logs my-harness --limit 200` if invocation fails

In-memory sessions disappear when the process ends; SQLite sessions depend on one instance's files. Before scaling a shared OAuth Harness across replicas, configure MySQL or PostgreSQL short-term memory and reachable private networking. See [Harness Sidecar](/productions/agentkit-cli/preview/en/agentkit-yaml#harness-sidecar) for managed Sidecar region, capacity, and authentication limits
