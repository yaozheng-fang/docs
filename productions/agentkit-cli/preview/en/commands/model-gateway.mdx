...
zh_link: "/productions/agentkit-cli/preview/zh/commands/model-gateway"
---
...
Configure cloud management credentials and a region before activating the gateway and creating a provider and consumer. Provider API keys authorize upstream model calls; consumer API keys authorize application access to the gateway. They are not interchangeable

The examples use Ark. Export `MODEL_PROVIDER_API_KEY` in the same terminal and verify model access in your account. For BytePlus, select `--provider byteplus` and use that region's gateway, ModelArk address, and model permissions
...
<Warning>
`consumer show` and `show-example` output includes the consumer API key. Keep full output out of public logs, tickets, and repositories; replace credentials before sharing examples
</Warning>
...
`show-example` prints an example without sending a model request. A successful answer after running it verifies gateway authentication, model permissions, and upstream credentials; the test incurs model usage
