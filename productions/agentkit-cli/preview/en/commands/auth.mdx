...
zh_link: "/productions/agentkit-cli/preview/zh/commands/auth"
---

The `auth` command group handles console and SSO authentication: log in through the browser and store short-lived STS credentials or only the user's OIDC session, clear the session, show the current identity, manage login profiles, and prepare CLI SSO login resources for an organization. `login`, `logout`, and `whoami` are also available as top-level commands (e.g. `agentkit login`).

Choose authentication for the task you will perform:

| Task | Authentication |
| --- | --- |
| Build, deploy, and manage resources | Console login, organization SSO that obtains STS, or access keys for the selected provider |
| Chat with an organization-shared Harness | Organization SSO with `--identity-only` and an administrator-published alias |
| Save login coordinates for later | `auth profile set`; saving a profile does not log in |

After browser authorization, run `agentkit whoami` to confirm the active identity. The correct identity still needs permissions for the target resource

## auth login

Authenticate through the selected cloud console or a supplied SSO address. SSO exchanges the OIDC login result for short-lived STS credentials. `--identity-only` stores only the user's OIDC session, allowing `harness invoke` to automatically forward the user `id_token` to a matching `custom_jwt` Runtime, but it does not create AgentKit management credentials. Generic `invoke run` calls to a `custom_jwt` Runtime still need an explicit `Authorization` header through `--headers`.
...
| `--no-open` | Print the login URL without opening a browser; use `--remote` for console login over SSH | `false` |
| `--console` | Use the selected cloud console even with an active SSO profile | `false` |
| `--remote` | Authorize console login on another device and paste the code into this terminal | `false` |
...
| `shared_harnesses` | Optional shared Harness alias map used by [`chat`](/productions/agentkit-cli/preview/en/commands/chat) to resolve Runtime IDs and HTTPS endpoints. | — |
...
Each discovery document may publish up to 32 `shared_harnesses` aliases. Each alias is 2 to 63 characters, starts and ends with a lowercase letter or digit, and may contain lowercase letters, digits, and hyphens in between. Each entry contains `runtime_id` and `endpoint`; `endpoint` must be a credential-free HTTPS origin without a path, query string, or fragment.
...
| Flag / argument | Description | Default |
| --- | --- | --- |
| _(no operation-specific options)_ | List locally saved login profiles | — |
...
## Console login and credential precedence

Without an address, login reuses the selected provider’s existing profile or opens console login when none exists. `--console` forces console login; `--remote` supports remote servers that cannot receive local browser callbacks. Console login cannot be combined with an SSO address or `--identity-only`
...
Volcengine and BytePlus console sessions are stored separately. `whoami` and `logout` select the current provider’s session. Valid environment credentials keep priority over cached login credentials

Commands that require cloud credentials can start browser login when credentials are missing in an interactive terminal. Global `--no-auto-login` disables it. CI, `--json`, `--raw`, `--yes`, and non-interactive terminals require credentials in advance. Local-only operations and `--dry-run` do not trigger automatic login

Logout clears CLI-stored sessions; it does not revoke access keys in environment variables or sign the browser out of the cloud console. If commands still access resources after logout, check whether the terminal still exports valid cloud credentials
