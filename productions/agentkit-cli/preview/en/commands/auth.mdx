...
Commands that require cloud credentials can start browser login when credentials are missing in an interactive terminal. Global `--no-auto-login` disables it. CI, JSON/raw output, `--yes`, and non-interactive terminals require credentials in advance. Local-only operations and dry runs do not trigger automatic login
