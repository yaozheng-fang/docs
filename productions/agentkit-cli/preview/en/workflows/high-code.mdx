...
<Note>
  Configure credentials first. `deploy` is a control-plane operation and requires AK/SK; [SSO login](/productions/agentkit-cli/preview/en/commands/auth) currently only covers invoking (`invoke`) a deployed runtime. See the Authentication section of the [Quickstart](/productions/agentkit-cli/preview/en/quickstart). For the full guide to writing agent code, see the [VeADK docs](/productions/veadk/preview/en/index).
</Note>
...
<Steps>
  ...
  <Step title="Build and deploy">
    ...
    <Tip>
      To inspect or hand-edit `agentkit.yaml` or the `Dockerfile` between scaffolding and building, run `agentkit deploy config`, `agentkit deploy build`, and `agentkit deploy apply` separately. See [Build & deploy](/productions/agentkit-cli/preview/en/commands/deploy).
    </Tip>
  </Step>
