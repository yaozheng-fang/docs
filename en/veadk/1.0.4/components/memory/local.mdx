...
For a full cross-session example, see [Long-term memory · Cross-session example](/en/veadk/1.0.4/components/memory/index#cross-session-example).
...
<Warning>
The `local` backend keeps memory only in process memory; data is lost when the process exits and cannot be shared across processes. For persistence or multi-instance sharing, use [VikingDB](/en/veadk/1.0.4/components/memory/vikingdb), [Mem0](/en/veadk/1.0.4/components/memory/mem0), [OpenSearch](/en/veadk/1.0.4/components/memory/opensearch), or [Redis](/en/veadk/1.0.4/components/memory/redis).
</Warning>
