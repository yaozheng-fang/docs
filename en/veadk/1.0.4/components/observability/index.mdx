...
VeADK runtime logs use Python's standard `logging` package and add a `trace_id` when an active span is available. See [Application logging](/en/veadk/1.0.4/components/observability/logging) for levels, formatting, and sensitive-data guidance.
...
| :--- | :--- | :--- | :--- |
| APMPlus | `APMPlusExporter` | Volcengine APMPlus, traces and metrics | [APMPlus](/en/veadk/1.0.4/components/observability/apmplus) |
| Cozeloop | `CozeloopExporter` | Cozeloop, trace observation and evaluation | [Cozeloop](/en/veadk/1.0.4/components/observability/cozeloop) |
| TLS | `TLSExporter` | Volcengine TLS log service | [TLS](/en/veadk/1.0.4/components/observability/tls) |
| In-memory | `InMemoryExporter` | In-process memory, local debugging and dumping | [In-memory exporter](/en/veadk/1.0.4/components/observability/inmemory) |
...
The exported JSON contains, for each span, its name, `span_id`, `trace_id`, start/end times, attributes, and parent span. See [In-memory exporter](/en/veadk/1.0.4/components/observability/inmemory) for details.
