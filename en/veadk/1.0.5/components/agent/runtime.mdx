...
- [function tools](/en/veadk/components/tools/custom-function) in `tools` are exposed as callable tools to the model;
- an [MCPToolset](/en/veadk/components/tools/custom-mcp) in `tools` supports tool discovery and invocation;
- [skills](/en/veadk/components/agent/skills) loaded through `SkillToolset` or VeADK's legacy entry are made available through the Codex skill mechanism.
