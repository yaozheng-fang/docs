...
- **Chat and debug**: multi-turn conversations showing thinking, tool calls, token usage and timing; conversations also render the [A2UI](/en/veadk/1.0.0/components/frontend/a2ui) rich-UI cards an agent returns.
...
A custom component has two halves that share one catalog id. For the backend half, see [A2UI](/en/veadk/1.0.0/components/frontend/a2ui#custom-components); the frontend half is below.
