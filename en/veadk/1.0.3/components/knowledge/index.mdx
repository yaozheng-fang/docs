...
| :--- | :--- | :--- | :--- | :--- |
| `local` | In-memory vector index | `extensions` + embedding | Local debugging (data lost on exit) | [Local](/en/veadk/1.0.3/components/knowledge/local) |
| `opensearch` | OpenSearch vector store | OpenSearch + `extensions` + embedding | Self-hosted vector search | [OpenSearch](/en/veadk/1.0.3/components/knowledge/opensearch) |
| `redis` | Redis vector store | Redis (RediSearch) + `extensions` + embedding | Low-latency self-hosted vector search | [Redis](/en/veadk/1.0.3/components/knowledge/redis) |
| `milvus` | Milvus collection | Milvus + `extensions` + embedding | Persistent self-hosted vector search | [Milvus](/en/veadk/1.0.3/components/knowledge/milvus) |
| `tos_vector` | TOS vector bucket | Volcengine account + `extensions` + embedding | Volcengine object-storage vector store | [TOS Vector](/en/veadk/1.0.3/components/knowledge/tos-vector) |
| `viking` | VikingDB knowledge base (managed) | Volcengine account | Recommended for production | [VikingDB](/en/veadk/1.0.3/components/knowledge/viking) |
| `context_search` | Context Search (managed) | Volcengine account | Recommended for production | [Context Search](/en/veadk/1.0.3/components/knowledge/context-search) |
| `openviking` | OpenViking resources | OpenViking service | Resource parsing and hierarchical retrieval | [OpenViking](/en/veadk/1.0.3/components/knowledge/openviking) |
