...
description: "The production-grade React app, and how to launch, build, and authenticate it from a single process"
---

VeADK ships a production-grade React frontend that renders [A2UI](/en/veadk/components/frontend/a2ui) streamed from an agent over the Google ADK API server. `veadk frontend` is a self-contained launcher: in the default mode it serves this React UI and the agent API from a single process on the same origin, so there is no separate backend to deploy and no cross-origin setup.
