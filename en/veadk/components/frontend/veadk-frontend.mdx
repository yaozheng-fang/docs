...
- **Chat and debug**: multi-turn conversations showing thinking, tool calls, token usage and timing; conversations also render the [A2UI](/en/veadk/components/frontend/a2ui) rich-UI cards an agent returns.
...
These restrictions do not replace authentication. Public deployments should still enable SSO or a trusted upstream gateway and use least-privilege cloud credentials. When you need only creation and management features, use the [Studio agent workbench](/en/veadk/components/frontend/studio).
...
A custom component has two halves that share one catalog id. For the backend half, see [A2UI](/en/veadk/components/frontend/a2ui#custom-components); the frontend half is below.
