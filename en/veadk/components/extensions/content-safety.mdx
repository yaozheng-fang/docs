...
<Note>
Roadmap: agent-level callbacks. `content_safety` also offers `before_agent_callback` and `after_agent_callback`, but they are not yet active — they perform no checks for now, so there is no need to wire them up. Agent-level input validation and context analysis are still planned.
</Note>
