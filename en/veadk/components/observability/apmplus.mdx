---
title: "APMPlus"
description: "Report agent trace and metrics data to the Volcengine APMPlus platform for trace observation and model metrics analysis"
---
...
<Note>
When `app_key` is not provided via environment variable, the exporter fetches an APMPlus token automatically through Volcengine authentication, so valid Volcengine credentials are required. The `endpoint` uses gRPC and connects with `insecure=True`.
</Note>
