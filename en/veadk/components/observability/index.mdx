---
title: "Observability"
description: "Trace the full execution of an agent to understand its behavior, debug issues, and analyze performance"
---
...
`OpentelemetryTracer` is the unified tracing entry point, located at `veadk.tracing.telemetry.opentelemetry_tracer`. It holds a list of exporters, registers each exporter's span processor with the global `TracerProvider` on initialization, and automatically attaches an in-memory exporter for local retention and dumping.
...
| :--- | :--- | :--- | :--- |
| APMPlus | `APMPlusExporter` | Volcengine APMPlus, traces and metrics | [APMPlus](/en/veadk/components/observability/apmplus) |
| Cozeloop | `CozeloopExporter` | Cozeloop, trace observation and evaluation | [Cozeloop](/en/veadk/components/observability/cozeloop) |
| TLS | `TLSExporter` | Volcengine TLS log service | [TLS](/en/veadk/components/observability/tls) |
| In-memory | `InMemoryExporter` | In-process memory, local debugging and dumping | [In-memory exporter](/en/veadk/components/observability/inmemory) |
...
<Note>
`InMemoryExporter` is managed automatically by `OpentelemetryTracer` and placed first in the span-processor chain so it records every span. Do not add it to the `exporters` list manually, or initialization raises `ValueError`.
</Note>
...
The exported JSON contains, for each span, its name, `span_id`, `trace_id`, start/end times, attributes, and parent span. See [In-memory exporter](/en/veadk/components/observability/inmemory) for details.
