...
- **Runtime auth**: on the Feishu path the runtime uses `key_auth` (the proxy holds the API key). For web login with per-user identity forwarding, see [Deploy a workbench with SSO login](/en/agentkit-cli/workflows/frontend-sso).
