...
<Steps>
  ...
  <Step title="Declare the frontend (edit agentkit.yaml)">
    Add a `frontend` block. Declare the user pool here only — the runtime's `custom_jwt` gateway auth is **derived from it automatically**, so you don't repeat `auth`. Secrets use `${VAR}` and stay out of the repo:
