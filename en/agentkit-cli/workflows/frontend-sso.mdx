...
<Steps>
  ...
  <Step title="Deploy">
    ...
    `agentkit deploy` needs no flags — it reads `agentkit.yaml` and, in order, builds and deploys the runtime (the image runs the veADK frontend, serving the UI and the agent API from one origin; `custom_jwt` derived from the user pool), then deploys the public frontend BFF on VeFaaS (reusing an existing serverless gateway, auto-fetching the client secret and auto-registering the callback `<frontend-url>/oauth2/callback`).
  </Step>
