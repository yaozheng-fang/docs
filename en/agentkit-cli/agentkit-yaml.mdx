...
#     websocket_url: ${WECOM_WEBSOCKET_URL:-wss://openws.work.weixin.qq.com}
...
The `im` block deploys a bot proxy to VeFaaS after the runtime to connect a messaging channel. Provide credentials via `${VAR}`. Feishu and WeCom are supported; enable either or both.
...
| `im.wecom.connection_mode` | Connection mode. `websocket` is the default and the only value supported today. |
| `im.wecom.bot_id` | WeCom bot id. |
| `im.wecom.secret` | WeCom bot secret. |
| `im.wecom.websocket_url` | WeCom WebSocket endpoint. Defaults to `wss://openws.work.weixin.qq.com`. |
| `im.wecom.send_thinking_message` | Whether to send an interim "thinking" message while the agent is working. Defaults to `true`. |
