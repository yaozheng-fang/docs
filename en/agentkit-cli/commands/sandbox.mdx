...
When `--tool-id` is omitted, the CLI looks for a sandbox tool of the requested type in the project: **exactly one is used automatically; when several exist the command fails and asks you to pass `--tool-id`**; when none exist it needs `--auto-create`. Having several `code` tools in a project is common, so run `agentkit tool list` to find the target id and pass it via `--tool-id`.
</Note>
